 ...
 * </p>
 * <p>
 * If using this plugin with JobStoreCMT, be sure to set the plugin property <em>wrapInUserTransaction</em> to true.
 * Also, if you have a positive <em>scanInterval</em> be sure to set
 ...
 * @author pl47ypus
 */
public class XMLSchedulingDataProcessorPlugin extends SchedulerPluginWithUserTransactionSupport implements
    FileScanListener {
  ...
  /**
   * The interval (in seconds) at which to scan for changes to the file. If the file has been changed, it is re-loaded
   * and parsed. The default value for the interval is 0, which disables scanning.
   * 
  ...
  /**
   * Whether or not initialization of the plugin should fail (throw an exception) if the file cannot be found. Default
   * is <code>true</code>.
   */
  ...
  /**
   * <p>
   * Called during creation of the <code>Scheduler</code> in order to give the <code>SchedulerPlugin</code> a chance to
   * initialize.
   * </p>
   * 
      ...
      if (jobFiles.isEmpty() == false) {
        ...
        while (iterator.hasNext()) {
          ...
          if (scanInterval > 0) {
            ...
            job.getJobDataMap().put(FileScanJob.FILE_SCAN_LISTENER_NAME,
                                    JOB_INITIALIZATION_PLUGIN_NAME + '_' + getName());

            getScheduler().scheduleJob(job, trig);
            getLog().debug("Scheduled file scan job for data file: {}, at interval: {}", jobFile.getFileName(),
                           scanInterval);
          }
  ...
  /**
   * Helper method for generating unique job/trigger name for the file scanning jobs (one per FileJob). The unique names
   * are saved in jobTriggerNameSet.
   */
  ...
  /**
   * Overriden to ignore <em>wrapInUserTransaction</em> because shutdown() does not interact with the
   * <code>Scheduler</code>.
   */
  @Override
  ...
  private void processFile(JobFile jobFile) {
    if (jobFile == null || !jobFile.getFileFound()) { return; }
