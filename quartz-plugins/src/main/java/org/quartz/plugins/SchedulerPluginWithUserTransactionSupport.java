...
/**
 * Base class for plugins that wish to support having their start and shutdown
 * methods run within a <code>UserTransaction</code>. This is often necessary if
 * using the JobStoreCMT and the plugin interacts with jobs/triggers.
 * 
 * <p>
 * The subclass should implement start(UserTransaction) and
 * shutdown(UserTransaction). The <code>UserTransaction</code> will be non-null
 * if property <em>wrapInUserTransaction</em> is set to true.
 * </p>
 * <p>
...
public abstract class SchedulerPluginWithUserTransactionSupport implements
     ...
     * 
     * <p>
     * If UserTransaction is not null, the plugin can call setRollbackOnly() on
     * it to signal that the wrapped transaction should rollback.
     * </p>
     * 
     * @param userTransaction
     *            The UserTranaction object used to provide a transaction around
     *            the start() operation. It will be null if
     *            <em>wrapInUserTransaction</em> is false or if the transaction
     ...
     * @param userTransaction
     *            The UserTranaction object used to provide a transaction around
     *            the shutdown() operation. It will be null if
     *            <em>wrapInUserTransaction</em> is false or if the transaction
     *            failed to be started.
     */
    ...
    public void initialize(String pname, Scheduler sched)
    ...
    /**
     * Wrap the start() and shutdown() methods in a UserTransaction. This is
     * necessary for some plugins if using the JobStoreCMT.
     */
    ...
    /**
     * Based on the value of <em>wrapInUserTransaction</em>, wraps the call to
     * start(UserTransaction) in a UserTransaction.
     */
    public void start() {
    ...
    /**
     * Based on the value of <em>wrapInUserTransaction</em>, wraps the call to
     * shutdown(UserTransaction) in a UserTransaction.
     */
    ...
    private UserTransaction startUserTransaction() {
        ...
        try {
            ...
            getLog().error(
                    "Failed to start UserTransaction for plugin: " + getName(),
                    t);
        }
    ...
    /**
     * If the given UserTransaction is not null, it is committed/rolledback, and
     * then returned to the UserTransactionHelper.
     */
    private void resolveUserTransaction(UserTransaction userTransaction) {
        if (userTransaction != null) {
            try {
                ...
                getLog().error(
                        "Failed to resolve UserTransaction for plugin: "
                                + getName(), t);
            } finally {
                UserTransactionHelper.returnUserTransaction(userTransaction);
            }
