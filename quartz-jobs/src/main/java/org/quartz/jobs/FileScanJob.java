...
/**
 * Inspects a file and compares whether it's "last modified date" has changed since the last time it was inspected. If
 * the file has been updated, the job invokes a "call-back" method on an identified <code>FileScanListener</code> that
 * can be found in the <code>SchedulerContext</code>.
 * 
 * @author jhouse
 * @author pl47ypus
...
public class FileScanJob implements Job {
  ...
  /**
   * <code>JobDataMap</code> key with which to specify the {@link org.quartz.jobs.FileScanListener} to be notified when
   * the file contents change.
   */
  public static final String  FILE_SCAN_LISTENER_NAME = "FILE_SCAN_LISTENER_NAME";

  /**
   * <code>JobDataMap</code> key with which to specify a <code>long</code> value that represents the minimum number of
   * milliseconds that must have past since the file's last modified time in order to consider the file new/altered.
   * This is necessary because another process may still be in the middle of writing to the file when the scan occurs,
   * and the file may therefore not yet be ready for processing.
   * <p>
   * If this parameter is not specified, a default value of <code>5000</code> (five seconds) will be used.
   * </p>
   */
  ...
  public void execute(JobExecutionContext context) throws JobExecutionException {
    ...
    if (fileName == null) { throw new JobExecutionException("Required parameter '" + FILE_NAME
                                                            + "' not found in merged JobDataMap"); }
    if (listenerName == null) { throw new JobExecutionException("Required parameter '" + FILE_SCAN_LISTENER_NAME
                                                                + "' not found in merged JobDataMap"); }

    FileScanListener listener = (FileScanListener) schedCtxt.get(listenerName);

    if (listener == null) { throw new JobExecutionException("FileScanListener named '" + listenerName
                                                            + "' not found in SchedulerContext"); }
