 ...
 *  
 * It will configure the scheduler with a DummyClassLoadHelper
 * (that extends CascadeClassLoadHelper) and using the XMLSchedulingDataProcessorPlugin
 * 
 * The job class configured in quartz_data.xml does not exist.
 * 
 * This test passes only if the plugin uses, like the SchedulerFactory does, the DummyClassLoadHelper
 * which is capable to load this imaginary job class 
 * (previous behavior was : always instantiates  the CacadeClassLoadHlper , not considering the SchedulerFactory classLoadHelper)
 * 
 * @author adahanne
 *
 */
public class QTZ225_SchedulerClassLoadHelperForPlugins_Test {
	...
	Logger log = LoggerFactory
			.getLogger(QTZ225_SchedulerClassLoadHelperForPlugins_Test.class);
