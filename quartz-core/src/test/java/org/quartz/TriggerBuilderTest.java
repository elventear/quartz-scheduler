...
public class TriggerBuilderTest extends TestCase {
  ...
  public void testTriggerBuilder() throws Exception {
    ...
    assertTrue("Unexpected trigger group: " + trigger.getKey().getGroup(),
               trigger.getKey().getGroup().equals(JobKey.DEFAULT_GROUP));
    ...
    trigger = newTrigger().withIdentity("t1").withDescription("my description").withPriority(2)
        .endAt(futureDate(10, IntervalUnit.WEEK)).startAt(stime).build();
  ...
  public void testTriggerBuilderWithEndTimePriorCurrrentTime() throws Exception {
    TriggerBuilder.newTrigger().withIdentity("some trigger name", "some trigger group")
        .forJob("some job name", "some job group").startAt(new Date(System.currentTimeMillis() - 200000000))
