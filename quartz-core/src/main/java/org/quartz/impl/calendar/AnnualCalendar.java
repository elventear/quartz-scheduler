...
/**
 * <p>
 * This implementation of the Calendar excludes a set of days of the year. You may use it to exclude bank holidays which
 * are on the same date every year.
 * </p>
 * 
...
public class AnnualCalendar extends BaseCalendar implements Calendar, Serializable {
  ...
  public boolean isDayExcluded(java.util.Calendar day) {
    ...
    if (day == null) { throw new IllegalArgumentException("Parameter day must not be null"); }

    // Check baseCalendar first
    if (!super.isTimeIncluded(day.getTime().getTime())) { return true; }
  ...
  public long getNextIncludedTime(long timeStamp) {
    ...
    if (isDayExcluded(day) == false) { return timeStamp; // return the original value
    }
