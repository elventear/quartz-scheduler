 ...
 * {@link org.quartz.impl.calendar.WeeklyCalendar WeeklyCalendar}. <CODE>WeeklyCalendar</CODE> defines a set of days
 * that are excluded <I>every week</I>. Likewise, <CODE>DailyCalendar</CODE> defines a set of times that are excluded
 * <I>every day</I>.
 * 
...
public class DailyCalendar extends BaseCalendar {
   ...
   * &quot;HH:MM[:SS[:mmm]]&quot; where:
   * <UL>
   * <LI>HH is the hour of the specified time. The hour should be specified using military (24-hour) time and must be in
   ...
   * </p>
   * 
   * @param baseCalendar the base calendar for this calendar instance &ndash; see {@link BaseCalendar} for more
   ...
   * @param rangeEndingMillis the millisecond of the start of the time range
   */
  public DailyCalendar(org.quartz.Calendar baseCalendar, int rangeStartingHourOfDay, int rangeStartingMinute,
                       int rangeStartingSecond, int rangeStartingMillis, int rangeEndingHourOfDay,
                       int rangeEndingMinute, int rangeEndingSecond, int rangeEndingMillis) {
    super(baseCalendar);
    setTimeRange(rangeStartingHourOfDay, rangeStartingMinute, rangeStartingSecond, rangeStartingMillis,
                 rangeEndingHourOfDay, rangeEndingMinute, rangeEndingSecond, rangeEndingMillis);
  }
  ...
  /**
   * Create a <CODE>DailyCalendar</CODE> with a time range defined by the specified <CODE>java.util.Calendar</CODE>s and
   * no <CODE>baseCalendar</CODE>. The Calendars are subject to the following considerations:
   * <UL>
   * <LI>Only the time-of-day fields of the specified Calendars will be used (the date fields will be ignored)</LI>
   * <LI>The starting time must be before the ending time of the defined time range. Note this means that a time range
   * may not cross daily boundaries (10PM - 2AM). <I>(because only time fields are are used, it is possible for two
   * Calendars to represent a valid time range and <CODE>rangeStartingCalendar.after(rangeEndingCalendar) == 
   *         true</CODE>)</I></LI>
   * </UL>
   * <p>
   * <b>Note:</b> This <CODE>DailyCalendar</CODE> will use the <code>{@link TimeZone#getDefault()}</code> time zone
   ...
   * </p>
   * 
   * @param rangeStartingCalendar a java.util.Calendar representing the starting time for the time range
  ...
  /**
   * Create a <CODE>DailyCalendar</CODE> with a time range defined by the specified <CODE>java.util.Calendar</CODE>s and
   * the specified <CODE>baseCalendar</CODE>. The Calendars are subject to the following considerations:
   * <UL>
   * <LI>Only the time-of-day fields of the specified Calendars will be used (the date fields will be ignored)</LI>
   * <LI>The starting time must be before the ending time of the defined time range. Note this means that a time range
   * may not cross daily boundaries (10PM - 2AM). <I>(because only time fields are are used, it is possible for two
   ...
   * <LI>The starting time must be before the ending time of the defined time range. Note this means that a time range
   * may not cross daily boundaries (10PM - 2AM). <I>(because only time value are are used, it is possible for the two
   * values to represent a valid time range and <CODE>rangeStartingTime &gt; 
   *         rangeEndingTime</CODE>)</I></LI>
   * </UL>
   * <p>
   * <b>Note:</b> This <CODE>DailyCalendar</CODE> will use the <code>{@link TimeZone#getDefault()}</code> time zone
   * unless an explicit time zone is set via <code>{@link BaseCalendar#setTimeZone(TimeZone)}</code>. You should use
   * <code>{@link #DailyCalendar(String, TimeZone, long, long)}</code> if you don't want the given
   * <code>rangeStartingTimeInMillis</code> and <code>rangeEndingTimeInMillis</code> to be evaluated in the default time
   * zone.
   * </p>
   * 
   ...
   * <b>Note:</b> This <CODE>DailyCalendar</CODE> will use the <code>{@link TimeZone#getDefault()}</code> time zone
   * unless an explicit time zone is set via <code>{@link BaseCalendar#setTimeZone(TimeZone)}</code>. You should use
   * <code>{@link #DailyCalendar(String, Calendar, TimeZone, long, long)}</code> if you don't want the given
   * <code>rangeStartingTimeInMillis</code> and <code>rangeEndingTimeInMillis</code> to be evaluated in the default time
   * zone.
   * </p>
   * 
   ...
   * </UL>
   * 
   * @param timeZone the time zone for of the <code>DailyCalendar</code> which will also be used to resolve the given
   *        start/end times.
   * @param rangeStartingTimeInMillis a long representing the starting time for the time range
   * @param rangeEndingTimeInMillis a long representing the ending time for the time range
   */
   ...
   * @param rangeEndingTimeInMillis a long representing the ending time for the time range
   */
  public DailyCalendar(org.quartz.Calendar baseCalendar, TimeZone timeZone, long rangeStartingTimeInMillis,
  ...
  public boolean isTimeIncluded(long timeInMillis) {
    if ((getBaseCalendar() != null) && (getBaseCalendar().isTimeIncluded(timeInMillis) == false)) { return false; }
  ...
  public void setTimeRange(String rangeStartingTimeString, String rangeEndingTimeString) {
    ...
    if ((rangeStartingTime.length < 2) || (rangeStartingTime.length > 4)) { throw new IllegalArgumentException(
                                                                                                               "Invalid time string '"
    ...
    if ((rEndingTime.length < 2) || (rEndingTime.length > 4)) { throw new IllegalArgumentException(
                                                                                                   "Invalid time string '"
  ...
  public void setTimeRange(int rangeStartingHourOfDay, int rangeStartingMinute, int rangeStartingSecond,
                           int rangeStartingMillis, int rangeEndingHourOfDay, int rangeEndingMinute,
    ...
    if (!startCal.before(endCal)) { throw new IllegalArgumentException(invalidTimeRange + rangeStartingHourOfDay + ":"
                                                                       + rangeStartingMinute + ":"
                                                                       + rangeStartingSecond + ":"
  ...
  private void validate(int hourOfDay, int minute, int second, int millis) {
    if (hourOfDay < 0 || hourOfDay > 23) { throw new IllegalArgumentException(invalidHourOfDay + hourOfDay); }
    if (minute < 0 || minute > 59) { throw new IllegalArgumentException(invalidMinute + minute); }
