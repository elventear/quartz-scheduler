...
/**
 * <p>
 * <code>JobStoreCMT</code> is meant to be used in an application-server environment that provides
 * container-managed-transactions. No commit / rollback will be1 handled by this class.
 * </p>
 * <p>
...
public class JobStoreCMT extends JobStoreSupport {
  ...
  public void initialize(ClassLoadHelper loadHelper, SchedulerSignaler signaler) throws SchedulerConfigException {
    ...
    if (nonManagedTxDsName == null) { throw new SchedulerConfigException(
  ...
  /**
   * Execute the given callback having optionally acquired the given lock. Because CMT assumes that the connection is
   * already part of a managed transaction, it does not attempt to commit or rollback the enclosing transaction.
   * 
   * @param lockName The name of the lock to acquire, for example "TRIGGER_ACCESS". If null, then no lock is acquired,
   *        but the txCallback is still executed in a transaction.
