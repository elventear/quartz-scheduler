...
/**
 * Provide thread/resource locking in order to protect resources from being altered by multiple threads at the same time
 * using a db row update.
 * <p>
 * <b>Note:</b> This Semaphore implementation is useful for databases that do not support row locking via
 * "SELECT FOR UPDATE" type syntax, for example Microsoft SQLServer (MSSQL).
 * </p>
 */
public class UpdateLockRowSemaphore extends DBSemaphore {
  ...
  public static final String UPDATE_FOR_LOCK = "UPDATE " + TABLE_PREFIX_SUBST + TABLE_LOCKS + " SET " + COL_LOCK_NAME
                                               + " = " + COL_LOCK_NAME + " WHERE " + COL_SCHEDULER_NAME + " = "
                                               + SCHED_NAME_SUBST + " AND " + COL_LOCK_NAME + " = ? ";

  public static final String INSERT_LOCK     = "INSERT INTO " + TABLE_PREFIX_SUBST + TABLE_LOCKS + "("
                                               + COL_SCHEDULER_NAME + ", " + COL_LOCK_NAME + ") VALUES ("
                                               + SCHED_NAME_SUBST + ", ?)";
  ...
  protected void executeSQL(Connection conn, final String lockName, final String expandedSQL,
    ...
    do {
      ...
      try {
        ...
        if (numUpdate < 1) {
          getLog().debug("Inserting new lock row for lock: '" + lockName + "' being obtained by thread: "
          ...
          if (res != 1) {
            ...
            throw new SQLException(Util.rtp("No row exists, and one could not be inserted in table "
                                                + TABLE_PREFIX_SUBST + TABLE_LOCKS + " for lock named: " + lockName,
                                            getTablePrefix(), getSchedulerNameLiteral()));
          }
