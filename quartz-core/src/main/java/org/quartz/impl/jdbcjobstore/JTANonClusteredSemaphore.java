...
/**
 * Provides in memory thread/resource locking that is JTA <code>{@link javax.transaction.Transaction}</code> aware. It
 * is most appropriate for use when using <code>{@link org.quartz.impl.jdbcjobstore.JobStoreCMT}</code> without
 * clustering.
 * <p>
 * This <code>Semaphore</code> implementation is <b>not</b> Quartz cluster safe.
 * </p>
 * <p>
 * When a lock is obtained/released but there is no active JTA <code>{@link javax.transaction.Transaction}</code>, then
 * this <code>Semaphore</code> operates just like <code>{@link org.quartz.impl.jdbcjobstore.SimpleSemaphore}</code>.
 * </p>
 * <p>
 * By default, this class looks for the <code>{@link javax.transaction.TransactionManager}</code> in JNDI under name
 * "java:TransactionManager". If this is not where your Application Server registers it, you can modify the JNDI lookup
 * location using the "transactionManagerJNDIName" property.
 * </p>
 * <p>
 * <b>IMPORTANT:</b> This Semaphore implementation is currently experimental. It has been tested a limited amount on
 * JBoss 4.0.3SP1. If you do choose to use it, any feedback would be most appreciated!
 * </p>
 * 
...
public class JTANonClusteredSemaphore implements Semaphore {
  ...
  public synchronized boolean obtainLock(Connection conn, String lockName) throws LockException {
    ...
    if (!isLockOwner(conn, lockName)) {
      ...
      log.debug("Lock '" + lockName + "' already owned by: " + Thread.currentThread().getName() + " -- but not owner!",
  ...
  /**
   * Helper method to get the current <code>{@link javax.transaction.Transaction}</code> from the
   * <code>{@link javax.transaction.TransactionManager}</code> in JNDI.
   * 
   * @return The current <code>{@link javax.transaction.Transaction}</code>, null if not currently in a transaction.
   */
  ...
  /**
   * Release the lock on the identified resource if it is held by the calling thread, unless currently in a JTA
   * transaction.
   */
   ...
   * transaction.
   * 
   * @param fromSynchronization True if this method is being invoked from <code>{@link Synchronization}</code> notified
   *        of the enclosing transaction having completed.
   * @throws LockException Thrown if there was a problem accessing the JTA <code>Transaction</code>. Only relevant if
   *         <code>fromSynchronization</code> is false.
   */
  ...
  /**
   * Helper class that is registered with the active <code>{@link javax.transaction.Transaction}</code> so that the lock
