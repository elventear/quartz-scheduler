...
public abstract class JobStoreSupport implements JobStore, Constants {
  ...
  private long                              misfireThreshold                           = 60000L;                         // one
                                                                                                                          // minute
  ...
  private Semaphore                         lockHandler                                = null;                           // set
                                                                                                                          // in
                                                                                                                          // initialize()
                                                                                                                          // method...
  ...
  private long                              dbRetryInterval                            = 15000L;                         // 15
                                                                                                                          // secs
  ...
  /**
   * <p>
   * Set the name of the <code>DataSource</code> that should be used for performing database functions.
   * </p>
   */
  ...
  /**
   * <p>
   * Get the name of the <code>DataSource</code> that should be used for performing database functions.
   * </p>
   */
  ...
  /**
   * <p>
   * Get the maximum number of misfired triggers that the misfire handling thread will try to recover at one time
   * (within one transaction). The default is 20.
   * </p>
   */
  ...
  /**
   * <p>
   * Set the maximum number of misfired triggers that the misfire handling thread will try to recover at one time
   * (within one transaction). The default is 20.
   * </p>
   */
  ...
  /**
   * Whether or not to obtain locks when inserting new jobs/triggers. Defaults to <code>true</code>, which is safest -
   * some db's (such as MS SQLServer) seem to require this to avoid deadlocks under high load, while others seem to do
   * fine without.
   * <p>
  ...
  /**
   * Don't call set autocommit(false) on connections obtained from the DataSource. This can be helpfull in a few
   ...
   * DB lock (to avoid possible race conditions on the trigger's db row). This is the behavior prior to Quartz 1.6.3,
   * but is considered unnecessary for most databases (due to the nature of the SQL update that is performed), and
   * therefore a superfluous performance hit.
   */
  ...
  /**
   * Whether or not the query and update to acquire a Trigger for firing should be performed after obtaining an explicit
   * DB lock. This is the behavior prior to Quartz 1.6.3, but is considered unnecessary for most databases, and
   * therefore a superfluous performance hit. However, if batch acquisition is used, it is important for this behavior
   ...
   * </p>
   * 
   * @param delegateClassName the delegate init string
   */
  ...
  /**
   * <p>
   * Called by the QuartzScheduler before the <code>JobStore</code> is used, in order to give it a chance to initialize.
   * </p>
   */
  public void initialize(ClassLoadHelper loadHelper, SchedulerSignaler signaler) throws SchedulerConfigException {

    if (dsName == null) { throw new SchedulerConfigException("DataSource name not set."); }
      ...
      if (getUseDBLocks()) {
        if (getDriverDelegateClass() != null && getDriverDelegateClass().equals(MSSQLDelegate.class.getName())) {
          if (getSelectWithLockSQL() == null) {
            String msSqlDflt = "SELECT * FROM {0}LOCKS WITH (UPDLOCK,ROWLOCK) WHERE " + COL_SCHEDULER_NAME
                               + " = {1} AND LOCK_NAME = ?";
  ...
  /**
   * Wrap the given <code>Connection</code> in a Proxy such that attributes that might be set will be restored before
   * the connection is closed (and potentially restored to a pool).
   */
  ...
  protected Connection getConnection() throws JobPersistenceException {
    ...
    try {
      ...
      throw new JobPersistenceException("Failed to obtain DB connection from data source '" + getDataSource() + "': "
                                        + sqle.toString(), sqle);
    } catch (Throwable e) {
      throw new JobPersistenceException("Failed to obtain DB connection from data source '" + getDataSource() + "': "
                                        + e.toString(), e);
    ...
    if (conn == null) { throw new JobPersistenceException("Could not get connection from DataSource '"
                                                          + getDataSource() + "'"); }
  ...
  protected void recoverJobs(Connection conn) throws JobPersistenceException {
    try {
      ...
      rows += getDelegate().updateTriggerStatesFromOtherStates(conn, STATE_PAUSED, STATE_PAUSED_BLOCKED,
      ...
      getLog().info("Recovering " + recoveringJobTriggers.size()
  ...
  protected RecoverMisfiredJobsResult recoverMisfiredJobs(Connection conn, boolean recovering)
    ...
    boolean hasMoreMisfiredTriggers = getDelegate().hasMisfiredTriggersInState(conn, STATE_WAITING, getMisfireTime(),
                                                                               maxMisfiresToHandleAtATime,
                                                                               misfiredTriggers);

    if (hasMoreMisfiredTriggers) {
      getLog().info("Handling the first " + misfiredTriggers.size()
    ...
    for (TriggerKey triggerKey : misfiredTriggers) {
      ...
      if (trig.getNextFireTime() != null && trig.getNextFireTime().getTime() < earliestNewTime) earliestNewTime = trig
  ...
  protected void storeTrigger(Connection conn, OperableTrigger newTrigger, JobDetail job, boolean replaceExisting,
    ...
    try {
      ...
      if (job == null) { throw new JobPersistenceException("The job (" + newTrigger.getJobKey()
  ...
  public boolean removeJob(final JobKey jobKey) throws JobPersistenceException {
    return ((Boolean) executeInLock(LOCK_TRIGGER_ACCESS, new TransactionCallback() {
      public Object execute(Connection conn) throws JobPersistenceException {
        return removeJob(conn, jobKey, true) ? Boolean.TRUE : Boolean.FALSE;
      }
    })).booleanValue();
  }
   ...
   */
  private boolean deleteTriggerAndChildren(Connection conn, TriggerKey key) throws SQLException,
  ...
  protected JobDetail retrieveJob(Connection conn, JobKey key) throws JobPersistenceException {
    try {
      JobDetail job = getDelegate().selectJobDetail(conn, key, getClassLoadHelper());
  ...
  /**
   * @see org.quartz.spi.JobStore#replaceTrigger(org.quartz.core.SchedulingContext, java.lang.String, java.lang.String,
   *      org.quartz.Trigger)
   */
  ...
  protected boolean replaceTrigger(Connection conn, TriggerKey key, OperableTrigger newTrigger)
      throws JobPersistenceException {
    try {
      // this must be called before we delete the trigger, obviously
      JobDetail job = getDelegate().selectJobForTrigger(conn, getClassLoadHelper(), key);
      ...
      if (!newTrigger.getJobKey().equals(job.getKey())) { throw new JobPersistenceException(
  ...
  public TriggerState getTriggerState(Connection conn, TriggerKey key) throws JobPersistenceException {
    try {
      ...
      if (ts.equals(STATE_DELETED)) { return TriggerState.NONE; }

      if (ts.equals(STATE_COMPLETE)) { return TriggerState.COMPLETE; }

      if (ts.equals(STATE_PAUSED)) { return TriggerState.PAUSED; }

      if (ts.equals(STATE_PAUSED_BLOCKED)) { return TriggerState.PAUSED; }

      if (ts.equals(STATE_ERROR)) { return TriggerState.ERROR; }

      if (ts.equals(STATE_BLOCKED)) { return TriggerState.BLOCKED; }
   ...
   *         replaceExisting is set to false.
   */
  public void storeCalendar(final String calName, final Calendar calendar, final boolean replaceExisting,
  ...
  protected void storeCalendar(Connection conn, String calName, Calendar calendar, boolean replaceExisting,
    ...
    try {
      ...
      if (existingCal && !replaceExisting) { throw new ObjectAlreadyExistsException("Calendar with name '" + calName
                                                                                    + "' already exists."); }
  ...
  /**
   * <p>
   * Get the number of <code>{@link org.quartz.Job}</code> s that are stored in the <code>JobStore</code>.
   * </p>
   */
  ...
  /**
   * <p>
   * Get the names of all of the <code>{@link org.quartz.Job}</code> s that matcher the given groupMatcher.
   * </p>
   * <p>
  ...
  /**
   * <p>
   * Get the names of all of the <code>{@link org.quartz.Trigger}</code> s that match the given group Matcher.
   * </p>
   * <p>
  ...
  protected String checkBlockedState(Connection conn, JobKey jobKey, String currentState)
    ...
    if ((currentState.equals(STATE_WAITING) == false) && (currentState.equals(STATE_PAUSED) == false)) { return currentState; }

    try {
      List<FiredTriggerRecord> lst = getDelegate().selectFiredTriggerRecordsByJob(conn, jobKey.getName(),
                                                                                  jobKey.getGroup());
      ...
      throw new JobPersistenceException("Couldn't determine if trigger should be in a blocked state '" + jobKey + "': "
                                        + e.getMessage(), e);
    }
  ...
  /**
   * <p>
   * Pause all of the <code>{@link org.quartz.Trigger}s</code> matching the given groupMatcher.
   * </p>
   * 
   ...
   */
  public Set<String> pauseTriggerGroup(Connection conn, GroupMatcher<TriggerKey> matcher)
      throws JobPersistenceException {

    try {

      getDelegate().updateTriggerGroupStateFromOtherStates(conn, matcher, STATE_PAUSED, STATE_ACQUIRED, STATE_WAITING,
                                                           STATE_WAITING);
   ...
   * @see #pauseTriggers(org.quartz.impl.matchers.GroupMatcher)
   */
  public Set<String> resumeTriggerGroup(Connection conn, GroupMatcher<TriggerKey> matcher)
      throws JobPersistenceException {
    ...
    try {
      ...
      /*
       * int res = getDelegate().updateTriggerGroupStateFromOtherState(conn, groupName, STATE_WAITING, PAUSED); if(res >
       * 0) { long misfireTime = System.currentTimeMillis(); if(getMisfireThreshold() > 0) misfireTime -=
       * getMisfireThreshold(); Key[] misfires = getDelegate().selectMisfiredTriggersInGroupInState(conn, groupName,
       * STATE_WAITING, misfireTime); List blockedTriggers = findTriggersToBeBlocked(conn, groupName); Iterator itr =
       * blockedTriggers.iterator(); while(itr.hasNext()) { Key key = (Key)itr.next();
       * getDelegate().updateTriggerState(conn, key.getName(), key.getGroup(), BLOCKED); } for(int i=0; i <
       * misfires.length; i++) { String newState = STATE_WAITING; if(blockedTriggers.contains(misfires[i])) newState =
       * BLOCKED; updateMisfiredTrigger(conn, misfires[i].getName(), misfires[i].getGroup(), newState, true); } }
       */
  ...
  public List<OperableTrigger> acquireNextTriggers(final long noLaterThan, final int maxCount, final long timeWindow)
    ...
    if (isAcquireTriggersWithinLock() || maxCount > 1) {
      return (List<OperableTrigger>) executeInNonManagedTXLock(LOCK_TRIGGER_ACCESS, new TransactionCallback() {
        public Object execute(Connection conn) throws JobPersistenceException {
      ...
      return (List<OperableTrigger>) executeInNonManagedTXLock(null, /*
                                                                      * passing null as lock name causes no lock to be
                                                                      * made
                                                                      */
                                                               new TransactionCallback() {
  ...
  public List<TriggerFiredResult> triggersFired(final List<OperableTrigger> triggers) throws JobPersistenceException {
    return (List<TriggerFiredResult>) executeInNonManagedTXLock(LOCK_TRIGGER_ACCESS, new TransactionCallback() {
      public Object execute(Connection conn) throws JobPersistenceException {
  ...
  protected TriggerFiredBundle triggerFired(Connection conn, OperableTrigger trigger) throws JobPersistenceException {
    ...
    return new TriggerFiredBundle(job, trigger, cal, trigger.getKey().getGroup()
        .equals(Scheduler.DEFAULT_RECOVERY_GROUP), new Date(), trigger.getPreviousFireTime(), prevFireTime,
  ...
  protected void triggeredJobComplete(Connection conn, OperableTrigger trigger, JobDetail jobDetail,
    ...
    try {
      ...
      if (jobDetail.isConcurrentExectionDisallowed()) {
        ...
        getDelegate().updateTriggerStatesForJobFromOtherState(conn, jobDetail.getKey(), STATE_PAUSED,
                                                              STATE_PAUSED_BLOCKED);
    ...
    synchronized (this) {
      if (null == delegate) {
        try {
          ...
          if (canUseProperties()) {
            Class<?>[] ctorParamTypes = new Class[] { Logger.class, String.class, String.class, String.class,
          ...
          throw new NoSuchDelegateException("Couldn't find delegate constructor: " + e.getMessage(), e);
  ...
  protected RecoverMisfiredJobsResult doRecoverMisfires() throws JobPersistenceException {
    ...
    try {
      ...
      int misfireCount = (getDoubleCheckLockMisfireHandler()) ? getDelegate()
          .countMisfiredTriggersInState(conn, STATE_WAITING, getMisfireTime()) : Integer.MAX_VALUE;
  ...
  protected List<SchedulerStateRecord> findFailedInstances(Connection conn) throws JobPersistenceException {
    try {
      ...
      if ((foundThisScheduler == false) && (firstCheckIn == false)) {
        ...
        getLog().warn("This scheduler instance (" + getInstanceId() + ") is still "
   ...
   */
  private List<SchedulerStateRecord> findOrphanedFailedInstances(Connection conn,
  ...
  protected long calcFailedIfAfter(SchedulerStateRecord rec) {
    return rec.getCheckinTimestamp() + Math.max(rec.getCheckinInterval(), (System.currentTimeMillis() - lastCheckin))
           + 7500L;
  }
  ...
  protected void clusterRecover(Connection conn, List<SchedulerStateRecord> failedInstances)
    ...
    if (failedInstances.size() > 0) {
      ...
      logWarnIfNonZero(failedInstances.size(), "ClusterManager: detected " + failedInstances.size()
        ...
        while (itr.hasNext()) {
          ...
          for (FiredTriggerRecord ftRec : firedTriggerRecs) {
            ...
            if (ftRec.getFireInstanceState().equals(STATE_ACQUIRED)) {
              ...
              if (jobExists(conn, jKey)) {
                SimpleTriggerImpl rcvryTrig = new SimpleTriggerImpl("recover_" + rec.getSchedulerInstanceId() + "_"
          ...
          for (TriggerKey triggerKey : triggerKeys) {
            ...
            if (getDelegate().selectTriggerState(conn, triggerKey).equals(STATE_COMPLETE)) {
              List<FiredTriggerRecord> firedTriggers = getDelegate().selectFiredTriggerRecords(conn,
                                                                                               triggerKey.getName(),
                                                                                               triggerKey.getGroup());
          ...
          logWarnIfNonZero(recoveredCount, "ClusterManager: ......Scheduled " + recoveredCount
  ...
  /**
   * Execute the given callback in a transaction. Depending on the JobStore, the surrounding transaction may be assumed
   * to be already present (managed).
   * <p>
  ...
  /**
   * Execute the given callback having aquired the given lock. Depending on the JobStore, the surrounding transaction
   * may be assumed to be already present (managed). This version is just a handy wrapper around executeInLock that
   * doesn't require a return value.
   * 
   * @param lockName The name of the lock to aquire, for example "TRIGGER_ACCESS". If null, then no lock is aquired, but
   *        the lockCallback is still executed in a transaction.
   * @see #executeInLock(String, TransactionCallback)
   */
  protected void executeInLock(final String lockName, final VoidTransactionCallback txCallback)
      throws JobPersistenceException {
  ...
  /**
   * Execute the given callback having aquired the given lock. Depending on the JobStore, the surrounding transaction
   * may be assumed to be already present (managed).
   * 
   * @param lockName The name of the lock to aquire, for example "TRIGGER_ACCESS". If null, then no lock is aquired, but
   *        the lockCallback is still executed in a transaction.
   */
  protected abstract Object executeInLock(String lockName, TransactionCallback txCallback)
      throws JobPersistenceException;

  /**
   * Execute the given callback having optionally aquired the given lock. This uses the non-managed transaction
   * connection. This version is just a handy wrapper around executeInNonManagedTXLock that doesn't require a return
   * value.
   * 
   * @param lockName The name of the lock to aquire, for example "TRIGGER_ACCESS". If null, then no lock is aquired, but
   ...
   */
  protected void executeInNonManagedTXLock(final String lockName, final VoidTransactionCallback txCallback)
      throws JobPersistenceException {
   ...
   *        the lockCallback is still executed in a non-managed transaction.
   */
  protected Object executeInNonManagedTXLock(String lockName, TransactionCallback txCallback)
      throws JobPersistenceException {
    boolean transOwner = false;
    Connection conn = null;
    try {
