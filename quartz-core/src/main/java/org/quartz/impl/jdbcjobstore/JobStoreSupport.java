...
public abstract class JobStoreSupport implements JobStore, Constants {
     ...
     * with the other instances of the cluster. -- Affects the rate of detecting
     * failed instances.
     * </p>
     */
    ...
    /**
     * Whether or not to obtain locks when inserting new jobs/triggers. Defaults
     * to <code>true</code>, which is safest - some db's (such as MS SQLServer)
     * seem to require this to avoid deadlocks under high load, while others
     ...
     * DataSource. This can be helpfull in a few situations, such as if you have
     * a driver that complains if it is called when it is already off.
     * 
     * @param b
     */
     ...
     * possible race conditions on the trigger's db row). This is the behavior
     * prior to Quartz 1.6.3, but is considered unnecessary for most databases
     * (due to the nature of the SQL update that is performed), and therefore a
     ...
     * before actually acquiring the lock to recover them. This should be set to
     * false if the majority of the time, there are are misfired Triggers.
     */
     ...
     * in order to give it a chance to initialize.
     * </p>
     */
    public void initialize(ClassLoadHelper loadHelper,
        ...
        if (getLockHandler() == null) {
            ...
            if (getUseDBLocks()) {
                if (getDriverDelegateClass() != null
                        && getDriverDelegateClass().equals(
                                MSSQLDelegate.class.getName())) {
                    if (getSelectWithLockSQL() == null) {
                        String msSqlDflt = "SELECT * FROM {0}LOCKS WITH (UPDLOCK,ROWLOCK) WHERE "
                                + COL_SCHEDULER_NAME
                                + " = {1} AND LOCK_NAME = ?";
                        getLog().info(
                ...
                setLockHandler(new StdRowLockSemaphore(getTablePrefix(),
                        getInstanceName(), getSelectWithLockSQL()));
            } else {
                getLog().info(
    ...
    public void schedulerStarted() throws SchedulerException {
        ...
        if (isClustered()) {
            ...
            if (initializersLoader != null)
                clusterManagementThread
                        .setContextClassLoader(initializersLoader);
        ...
        getLog().debug(
                "JobStore background threads started (as scheduler was started).");
    }
     ...
     * that might be set will be restored before the connection is closed (and
    ...
    protected void recoverJobs(Connection conn) throws JobPersistenceException {
        try {
            ...
            List<TriggerKey> cts = getDelegate().selectTriggersInState(conn,
                    STATE_COMPLETE);
    ...
    protected static class RecoverMisfiredJobsResult {
        public static final RecoverMisfiredJobsResult NO_OP = new RecoverMisfiredJobsResult(
    ...
    protected RecoverMisfiredJobsResult recoverMisfiredJobs(Connection conn,
            boolean recovering) throws JobPersistenceException, SQLException {

        // If recovering, we want to handle all of the misfired
        // triggers right away.
        int maxMisfiresToHandleAtATime = (recovering) ? -1
                : getMaxMisfiresToHandleAtATime();

        List<TriggerKey> misfiredTriggers = new LinkedList<TriggerKey>();
        long earliestNewTime = Long.MAX_VALUE;
        // We must still look for the MISFIRED state in case triggers were left
        // in this state when upgrading to this version that does not support
        // it.
        boolean hasMoreMisfiredTriggers = getDelegate()
                .hasMisfiredTriggersInState(conn, STATE_WAITING,
                        getMisfireTime(), maxMisfiresToHandleAtATime,
                        misfiredTriggers);
        ...
        for (TriggerKey triggerKey : misfiredTriggers) {
            ...
            doUpdateOfMisfiredTrigger(conn, trig, false, STATE_WAITING,
                    recovering);

            if (trig.getNextFireTime() != null
                    && trig.getNextFireTime().getTime() < earliestNewTime)
                earliestNewTime = trig.getNextFireTime().getTime();
        }

        return new RecoverMisfiredJobsResult(hasMoreMisfiredTriggers,
                misfiredTriggers.size(), earliestNewTime);
    }

    protected boolean updateMisfiredTrigger(Connection conn,
            TriggerKey triggerKey, String newStateIfNotComplete,
            boolean forceState) // TODO: probably get rid of this
            throws JobPersistenceException {
        try {
            ...
            doUpdateOfMisfiredTrigger(conn, trig, forceState,
                    newStateIfNotComplete, false);

            return true;

        } catch (Exception e) {
            throw new JobPersistenceException(
                    "Couldn't update misfired trigger '" + triggerKey + "': "
                            + e.getMessage(), e);
        }
    }

    private void doUpdateOfMisfiredTrigger(Connection conn,
            OperableTrigger trig, boolean forceState,
            String newStateIfNotComplete, boolean recovering)
            throws JobPersistenceException {
        Calendar cal = null;
        ...
        if (trig.getNextFireTime() == null) {
            storeTrigger(conn, trig, null, true, STATE_COMPLETE, forceState,
                    recovering);
    ...
    public void storeJobAndTrigger(final JobDetail newJob,
        ...
        executeInLock((isLockOnInsert()) ? LOCK_TRIGGER_ACCESS : null,
                new VoidTransactionCallback() {
                    public void execute(Connection conn)
                            throws JobPersistenceException {
    ...
    public void storeJob(final JobDetail newJob, final boolean replaceExisting)
            throws ObjectAlreadyExistsException, JobPersistenceException {
        executeInLock(
                (isLockOnInsert() || replaceExisting) ? LOCK_TRIGGER_ACCESS
                        : null, new VoidTransactionCallback() {
                    public void execute(Connection conn)
                            throws JobPersistenceException {
     ...
     * </p>
     */
    protected void storeJob(Connection conn, JobDetail newJob,
    ...
    protected boolean jobExists(Connection conn, JobKey jobKey)
        ...
        try {
            ...
            throw new JobPersistenceException(
                    "Couldn't determine job existence (" + jobKey + "): "
                            + e.getMessage(), e);
        }
    }
     ...
     * </p>
     */
    protected void storeTrigger(Connection conn, OperableTrigger newTrigger,
            JobDetail job, boolean replaceExisting, String state,
        ...
        try {
            ...
            if (!forceState) {
                ...
                if (shouldBepaused
                        && (state.equals(STATE_WAITING) || state
                                .equals(STATE_ACQUIRED))) {
                    state = STATE_PAUSED;
                }
            }
            ...
            if (job == null) {
                job = getDelegate().selectJobDetail(conn,
                        newTrigger.getJobKey(), getClassLoadHelper());
            }
            if (job == null) {
            ...
            throw new JobPersistenceException("Couldn't store trigger '"
                    + newTrigger.getKey() + "' for '" + newTrigger.getJobKey()
                    + "' job:" + e.getMessage(), e);
        }
    }
    ...
    protected boolean triggerExists(Connection conn, TriggerKey key)
        ...
        try {
            ...
            throw new JobPersistenceException(
                    "Couldn't determine trigger existence (" + key + "): "
                            + e.getMessage(), e);
        }
    }
    ...
    public boolean removeJob(final JobKey jobKey)
            throws JobPersistenceException {
        return ((Boolean) executeInLock(LOCK_TRIGGER_ACCESS,
                new TransactionCallback() {
                    public Object execute(Connection conn)
                            throws JobPersistenceException {
                        return removeJob(conn, jobKey, true) ? Boolean.TRUE
                                : Boolean.FALSE;
                    }
                })).booleanValue();
    }
    ...
    protected boolean removeJob(Connection conn, final JobKey jobKey,
            boolean activeDeleteSafe) throws JobPersistenceException {

        try {
            List<TriggerKey> jobTriggers = getDelegate()
                    .selectTriggerKeysForJob(conn, jobKey);
        ...
        return ((Boolean) executeInLock(LOCK_TRIGGER_ACCESS,
                new TransactionCallback() {
                    public Object execute(Connection conn)
                        ...
                        // TODO: make this more efficient with a true bulk
                        // operation...
                        for (JobKey jobKey : jobKeys)
                            allFound = removeJob(conn, jobKey, true)
                                    && allFound;
    ...
    public void storeJobsAndTriggers(
            final Map<JobDetail, Set<Trigger>> triggersAndJobs,
            final boolean replace) throws ObjectAlreadyExistsException,
            JobPersistenceException {

        executeInLock((isLockOnInsert() || replace) ? LOCK_TRIGGER_ACCESS
                : null, new VoidTransactionCallback() {
            public void execute(Connection conn) throws JobPersistenceException {
                ...
                for (JobDetail job : triggersAndJobs.keySet()) {
                    ...
                    for (Trigger trigger : triggersAndJobs.get(job)) {
                        storeTrigger(conn, (OperableTrigger) trigger, job,
                                replace, Constants.STATE_WAITING, false, false);
                    }
                }
            }
        });
    ...
    /**
     * Delete a trigger, its listeners, and its Simple/Cron/BLOB sub-table
     * entry.
     * 
     * @see #removeJob(Connection, SchedulingContext, String, String, boolean)
     * @see #removeTrigger(Connection, SchedulingContext, String, String)
     * @see #replaceTrigger(Connection, SchedulingContext, String, String,
     *      Trigger)
     */
    ...
    public boolean removeTrigger(final TriggerKey triggerKey)
        ...
        return ((Boolean) executeInLock(LOCK_TRIGGER_ACCESS,
                new TransactionCallback() {
                    public Object execute(Connection conn)
                            throws JobPersistenceException {
                        return removeTrigger(conn, triggerKey) ? Boolean.TRUE
                                : Boolean.FALSE;
                    }
                })).booleanValue();
    }
    ...
    public boolean replaceTrigger(final TriggerKey triggerKey,
        ...
        return ((Boolean) executeInLock(LOCK_TRIGGER_ACCESS,
                new TransactionCallback() {
                    public Object execute(Connection conn)
                            throws JobPersistenceException {
                        return replaceTrigger(conn, triggerKey, newTrigger) ? Boolean.TRUE
                                : Boolean.FALSE;
                    }
                })).booleanValue();
    }

    protected boolean replaceTrigger(Connection conn, TriggerKey key,
        ...
        try {
            ...
            storeTrigger(conn, newTrigger, job, false, STATE_WAITING, false,
                    false);
    ...
    public OperableTrigger retrieveTrigger(final TriggerKey triggerKey)
            throws JobPersistenceException {
        return (OperableTrigger) executeWithoutLock( // no locks necessary for
                                                     // read...
        new TransactionCallback() {
            public Object execute(Connection conn)
                    throws JobPersistenceException {
    ...
    public TriggerState getTriggerState(final TriggerKey triggerKey)
            throws JobPersistenceException {
        return (TriggerState) executeWithoutLock( // no locks necessary for
                                                  // read...
        new TransactionCallback() {
            public Object execute(Connection conn)
                    throws JobPersistenceException {
    ...
    public TriggerState getTriggerState(Connection conn, TriggerKey key)
        ...
        try {
            ...
            throw new JobPersistenceException(
                    "Couldn't determine state of trigger (" + key + "): "
                            + e.getMessage(), e);
        }
    }
     ...
     */
    public void storeCalendar(final String calName, final Calendar calendar,
            final boolean replaceExisting, final boolean updateTriggers)
            throws ObjectAlreadyExistsException, JobPersistenceException {
        executeInLock(
                (isLockOnInsert() || updateTriggers) ? LOCK_TRIGGER_ACCESS
                        : null, new VoidTransactionCallback() {
                    public void execute(Connection conn)
                            throws JobPersistenceException {
                        storeCalendar(conn, calName, calendar, replaceExisting,
                                updateTriggers);
                    }
                });
    }

    protected void storeCalendar(Connection conn, String calName,
        ...
        try {
            ...
            if (existingCal) {
                ...
                if (updateTriggers) {
                    List<OperableTrigger> trigs = getDelegate()
                            .selectTriggersForCalendar(conn, calName);

                    for (OperableTrigger trigger : trigs) {
                        trigger.updateWithNewCalendar(calendar,
                                getMisfireThreshold());
                        storeTrigger(conn, trigger, null, true, STATE_WAITING,
                                false, false);
                    }
                }
            } else {
    ...
    public boolean removeCalendar(final String calName)
        ...
        return ((Boolean) executeInLock(LOCK_TRIGGER_ACCESS,
                new TransactionCallback() {
                    public Object execute(Connection conn)
                            throws JobPersistenceException {
                        return removeCalendar(conn, calName) ? Boolean.TRUE
                                : Boolean.FALSE;
                    }
                })).booleanValue();
    }
    ...
    public Set<JobKey> getJobKeys(final GroupMatcher<JobKey> matcher)
            throws JobPersistenceException {
        return (Set<JobKey>) executeWithoutLock( // no locks necessary for
                                                 // read...
        new TransactionCallback() {
            public Object execute(Connection conn)
                    throws JobPersistenceException {
    ...
    protected boolean checkExists(Connection conn, JobKey jobKey)
        ...
        try {
            ...
            throw new JobPersistenceException(
                    "Couldn't check for existence of job: " + e.getMessage(), e);
        }
    }
    ...
    protected void clearAllSchedulingData(Connection conn)
        ...
        try {
            ...
            throw new JobPersistenceException(
                    "Error clearing scheduling data: " + e.getMessage(), e);
        }
    }
    ...
    public Set<TriggerKey> getTriggerKeys(final GroupMatcher<TriggerKey> matcher)
            throws JobPersistenceException {
        return (Set<TriggerKey>) executeWithoutLock( // no locks necessary for
                                                     // read...
        new TransactionCallback() {
            public Object execute(Connection conn)
                    throws JobPersistenceException {
    ...
    public List<String> getJobGroupNames() throws JobPersistenceException {
        return (List<String>) executeWithoutLock( // no locks necessary for
                                                  // read...
        new TransactionCallback() {
            public Object execute(Connection conn)
                    throws JobPersistenceException {
    ...
    public List<OperableTrigger> getTriggersForJob(final JobKey jobKey)
            throws JobPersistenceException {
        return (List<OperableTrigger>) executeWithoutLock( // no locks necessary
                                                           // for read...
        new TransactionCallback() {
            public Object execute(Connection conn)
                    throws JobPersistenceException {
    ...
    protected List<OperableTrigger> getTriggersForJob(Connection conn,
            JobKey key) throws JobPersistenceException {
    ...
    public void pauseTrigger(Connection conn, TriggerKey triggerKey)
        ...
        try {
            String oldState = getDelegate()
                    .selectTriggerState(conn, triggerKey);

            if (oldState.equals(STATE_WAITING)
                    || oldState.equals(STATE_ACQUIRED)) {

                getDelegate()
                        .updateTriggerState(conn, triggerKey, STATE_PAUSED);
    ...
    public Set<String> pauseJobs(final GroupMatcher<JobKey> matcher)
            throws JobPersistenceException {
        return (Set<String>) executeInLock(LOCK_TRIGGER_ACCESS,
                new TransactionCallback() {
                    public Set<String> execute(final Connection conn)
                            throws JobPersistenceException {
                        Set<String> groupNames = new HashSet<String>();
                        Set<JobKey> jobNames = getJobNames(conn, matcher);

                        for (JobKey jobKey : jobNames) {
                            List<OperableTrigger> triggers = getTriggersForJob(
                                    conn, jobKey);
                            for (OperableTrigger trigger : triggers) {
                                pauseTrigger(conn, trigger.getKey());
                            }
    ...
    /**
     * Determines if a Trigger for the given job should be blocked. State can
     * only transition to STATE_PAUSED_BLOCKED/BLOCKED from PAUSED/STATE_WAITING
     * respectively.
     * 
     * @return STATE_PAUSED_BLOCKED, BLOCKED, or the currentState.
     */
    protected String checkBlockedState(Connection conn, JobKey jobKey,
            String currentState) throws JobPersistenceException {
        ...
        try {
            List<FiredTriggerRecord> lst = getDelegate()
                    .selectFiredTriggerRecordsByJob(conn, jobKey.getName(),
                            jobKey.getGroup());

            if (lst.size() > 0) {
                FiredTriggerRecord rec = lst.get(0);
                if (rec.isJobDisallowsConcurrentExecution()) { // TODO: worry
                                                               // about
                                                               // failed/recovering/volatile
                                                               // job states?
                    return (STATE_PAUSED.equals(currentState)) ? STATE_PAUSED_BLOCKED
                            : STATE_BLOCKED;
                }
            }
    ...
    public void resumeTrigger(Connection conn, TriggerKey key)
        ...
        try {
            ...
            String newState = checkBlockedState(conn, status.getJobKey(),
                    STATE_WAITING);
            ...
            if (!misfired) {
                if (blocked) {
                    getDelegate().updateTriggerStateFromOtherState(conn, key,
                            newState, STATE_PAUSED_BLOCKED);
                } else {
                    getDelegate().updateTriggerStateFromOtherState(conn, key,
            ...
            throw new JobPersistenceException("Couldn't resume trigger '" + key
                    + "': " + e.getMessage(), e);
        }
    }
    ...
    public Set<String> resumeJobs(final GroupMatcher<JobKey> matcher)
        ...
        return (Set<String>) executeInLock(LOCK_TRIGGER_ACCESS,
                new TransactionCallback() {
                    public Set<String> execute(Connection conn)
                            throws JobPersistenceException {
    ...
    public Set<String> pauseTriggerGroup(Connection conn,
        ...
        try {
            ...
            getDelegate().updateTriggerGroupStateFromOtherStates(conn, matcher,
                    STATE_PAUSED, STATE_ACQUIRED, STATE_WAITING, STATE_WAITING);

            getDelegate().updateTriggerGroupStateFromOtherState(conn, matcher,
                    STATE_PAUSED_BLOCKED, STATE_BLOCKED);

            List<String> groups = getDelegate().selectTriggerGroups(conn,
                    matcher);

            // make sure to account for an exact group match for a group that
            // doesn't yet exist
            StringMatcher.StringOperatorName operator = matcher
                    .getCompareWithOperator();
            if (operator.equals(StringOperatorName.EQUALS)
                    && !groups.contains(matcher.getCompareToValue())) {
    ...
    public Set<String> getPausedTriggerGroups() throws JobPersistenceException {
        return (Set<String>) executeWithoutLock( // no locks necessary for
                                                 // read...
        new TransactionCallback() {
            public Object execute(Connection conn)
                    throws JobPersistenceException {
    ...
    public Set<String> getPausedTriggerGroups(Connection conn)
        ...
        try {
            ...
            throw new JobPersistenceException(
                    "Couldn't determine paused trigger groups: "
                            + e.getMessage(), e);
        }
    }
    ...
    public Set<String> resumeTriggerGroup(Connection conn,
        ...
        try {
             ...
             * 
             * List blockedTriggers = findTriggersToBeBlocked(conn, groupName);
             * 
             * Iterator itr = blockedTriggers.iterator(); while(itr.hasNext()) {
             * Key key = (Key)itr.next(); getDelegate().updateTriggerState(conn,
             * key.getName(), key.getGroup(), BLOCKED); }
             * 
             * for(int i=0; i < misfires.length; i++) { String newState =
             * STATE_WAITING; if(blockedTriggers.contains(misfires[i])) newState
             * = BLOCKED; updateMisfiredTrigger(conn, misfires[i].getName(),
        ...
        if (isAcquireTriggersWithinLock() || maxCount > 1) {
            return (List<OperableTrigger>) executeInNonManagedTXLock(
                    LOCK_TRIGGER_ACCESS, new TransactionCallback() {
                        public Object execute(Connection conn)
                                throws JobPersistenceException {
                            return acquireNextTrigger(conn, noLaterThan,
                                    maxCount, timeWindow);
                        }
                    });
        } else {
            return (List<OperableTrigger>) executeInNonManagedTXLock(null, /*
                                                                            * passing
                                                                            * null
                                                                            * as
                                                                            * lock
        ...
        do {
            ...
            try {
                ...
                for (TriggerKey triggerKey : keys) {
                    ...
                    OperableTrigger nextTrigger = retrieveTrigger(conn,
                            triggerKey);
                    if (nextTrigger == null) {
                    ...
                    // and it has already been added to result, then
                    // put it back into the timeTriggers set and continue to
                    // search for next trigger.
                    JobKey jobKey = nextTrigger.getJobKey();
                    JobDetail job = getDelegate().selectJobDetail(conn, jobKey,
                            getClassLoadHelper());
                    ...
                    // If our trigger was no longer in the expected state, try a
                    // new one.
                    int rowsUpdated = getDelegate()
                            .updateTriggerStateFromOtherState(conn, triggerKey,
                    ...
                    if (firstAcquiredTriggerFireTime == 0)
                        firstAcquiredTriggerFireTime = nextTrigger
                                .getNextFireTime().getTime();
                }

                // if we didn't end up with any trigger to fire from that first
                // batch, try again for another batch. We allow with a max retry
                // count.
    ...
    public List<TriggerFiredResult> triggersFired(
            final List<OperableTrigger> triggers)
            throws JobPersistenceException {
        return (List<TriggerFiredResult>) executeInNonManagedTXLock(
                LOCK_TRIGGER_ACCESS, new TransactionCallback() {
                    public Object execute(Connection conn)
                            throws JobPersistenceException {
                        ...
                        for (OperableTrigger trigger : triggers) {
                            try {
                                TriggerFiredBundle bundle = triggerFired(conn,
                                        trigger);
    ...
    protected TriggerFiredBundle triggerFired(Connection conn,
        ...
        try {
            getDelegate().updateFiredTrigger(conn, trigger, STATE_EXECUTING,
                    job);
        } catch (SQLException e) {
        ...
        if (job.isConcurrentExectionDisallowed()) {
            ...
            try {
                getDelegate().updateTriggerStatesForJobFromOtherState(conn,
                        job.getKey(), STATE_BLOCKED, STATE_WAITING);
                getDelegate().updateTriggerStatesForJobFromOtherState(conn,
                        job.getKey(), STATE_BLOCKED, STATE_ACQUIRED);
                getDelegate().updateTriggerStatesForJobFromOtherState(conn,
                        job.getKey(), STATE_PAUSED_BLOCKED, STATE_PAUSED);
            } catch (SQLException e) {
                throw new JobPersistenceException(
        ...
        return new TriggerFiredBundle(job, trigger, cal, trigger.getKey()
                .getGroup().equals(Scheduler.DEFAULT_RECOVERY_GROUP),
    ...
    public void triggeredJobComplete(final OperableTrigger trigger,
        ...
        executeInNonManagedTXLock(LOCK_TRIGGER_ACCESS,
                new VoidTransactionCallback() {
                    public void execute(Connection conn)
                            throws JobPersistenceException {
                        triggeredJobComplete(conn, trigger, jobDetail,
                                triggerInstCode);
                    }
                });
    }
    ...
    protected void triggeredJobComplete(Connection conn,
        ...
        try {
            if (triggerInstCode == CompletedExecutionInstruction.DELETE_TRIGGER) {
                ...
                getLog().info(
                        "Trigger " + trigger.getKey() + " set to ERROR state.");
                getDelegate().updateTriggerState(conn, trigger.getKey(),
                        STATE_ERROR);
                signalSchedulingChangeOnTxCompletion(0L);
                ...
                getLog().info(
                        "All triggers of Job " + trigger.getKey()
                                + " set to ERROR state.");
                getDelegate().updateTriggerStatesForJob(conn,
            ...
            if (jobDetail.isConcurrentExectionDisallowed()) {
                getDelegate().updateTriggerStatesForJobFromOtherState(conn,
                        jobDetail.getKey(), STATE_WAITING, STATE_BLOCKED);

                getDelegate().updateTriggerStatesForJobFromOtherState(conn,
                        jobDetail.getKey(), STATE_PAUSED, STATE_PAUSED_BLOCKED);
            ...
            if (null == delegate) {
                try {
                    if (delegateClassName != null) {
                        delegateClass = getClassLoadHelper().loadClass(
                                delegateClassName, DriverDelegate.class);
                    }

                    // TODO: the current method of instantiating and
                    // initializing delegates is really sucky
                    // probably all constructor args should be moved to the
                    // initialize method and/or use
                    // the TablePrefixAware interface to set some things (and
                    // rename that interface to
                    // something more apt), etc. etc.

                    Constructor<?> ctor = null;
                    Object[] ctorParams = null;
                    if (canUseProperties()) {
                        Class<?>[] ctorParamTypes = new Class[] { Logger.class,
                                String.class, String.class, String.class,
                                ClassLoadHelper.class, Boolean.class };
                        ctor = delegateClass.getConstructor(ctorParamTypes);
                        ctorParams = new Object[] { getLog(), tablePrefix,
                                instanceName, instanceId, getClassLoadHelper(),
                                Boolean.valueOf(canUseProperties()) };
                    } else {
                        Class<?>[] ctorParamTypes = new Class[] { Logger.class,
                                String.class, String.class, String.class,
                    ...
                    throw new NoSuchDelegateException(
                            "Couldn't find delegate constructor: "
                                    + e.getMessage(), e);
                } catch (InstantiationException e) {
                    throw new NoSuchDelegateException(
                            "Couldn't create delegate: " + e.getMessage(), e);
                    ...
                    throw new NoSuchDelegateException(
                            "Couldn't load delegate class: " + e.getMessage(),
                            e);
                }
            }
            return delegate;
    ...
    protected RecoverMisfiredJobsResult doRecoverMisfires()
        ...
        try {
            ...
            int misfireCount = (getDoubleCheckLockMisfireHandler()) ? getDelegate()
                    .countMisfiredTriggersInState(conn, STATE_WAITING,
                            getMisfireTime()) : Integer.MAX_VALUE;

            if (misfireCount == 0) {
                getLog().debug(
                        "Found 0 triggers that missed their scheduled fire-time.");
            } else {
                transOwner = getLockHandler().obtainLock(conn,
                        LOCK_TRIGGER_ACCESS);
            ...
            rollbackConnection(conn);
            throw new JobPersistenceException(
                    "Database error recovering from misfires.", e);
    ...
    protected void signalSchedulingChangeOnTxCompletion(
            long candidateNewNextFireTime) {
    ...
    protected void signalSchedulingChangeImmediately(
            long candidateNewNextFireTime) {
    ...
    protected boolean doCheckin() throws JobPersistenceException {
        ...
        Connection conn = getNonManagedTXConnection();
        try {
            // Other than the first time, always checkin first to make sure
            // there is
            // work to be done before we acquire the lock (since that is
            // expensive,
            ...
            if (firstCheckIn || (failedRecords.size() > 0)) {
                ...
                // The first time through, we also need to make sure we
                // update/create our state record
    ...
    /**
     * Get a list of all scheduler instances in the cluster that may have
     * failed. This includes this scheduler if it is checking in for the first
     * time.
     */
    protected List<SchedulerStateRecord> findFailedInstances(Connection conn)
        ...
        try {
            ...
            List<SchedulerStateRecord> states = getDelegate()
                    .selectSchedulerStateRecords(conn, null);
            ...
            if (firstCheckIn) {
                failedInstances
                        .addAll(findOrphanedFailedInstances(conn, states));
            }

            // If not the first time but we didn't find our own instance, then
            // Someone must have done recovery for us.
            if ((foundThisScheduler == false) && (firstCheckIn == false)) {
                // TODO: revisit when handle self-failed-out impl'ed (see TODO
                // in clusterCheckIn() below)
                getLog().warn(
            ...
            throw new JobPersistenceException(
                    "Failure identifying failed instances when checking-in: "
                            + e.getMessage(), e);
        }
    }
    ...
    private List<SchedulerStateRecord> findOrphanedFailedInstances(
            Connection conn, List<SchedulerStateRecord> schedulerStateRecords)
            throws SQLException, NoSuchDelegateException {
        List<SchedulerStateRecord> orphanedInstances = new ArrayList<SchedulerStateRecord>();

        Set<String> allFiredTriggerInstanceNames = getDelegate()
                .selectFiredTriggerInstanceNames(conn);
        if (allFiredTriggerInstanceNames.isEmpty() == false) {
            for (SchedulerStateRecord rec : schedulerStateRecords) {

                allFiredTriggerInstanceNames.remove(rec
            ...
            for (String inst : allFiredTriggerInstanceNames) {
                ...
                getLog().warn(
                        "Found orphaned fired triggers for instance: "
                                + orphanedInstance.getSchedulerInstanceId());
            }
        }
    ...
    protected long calcFailedIfAfter(SchedulerStateRecord rec) {
        return rec.getCheckinTimestamp()
                + Math.max(rec.getCheckinInterval(),
                        (System.currentTimeMillis() - lastCheckin)) + 7500L;
    }
    ...
    protected List<SchedulerStateRecord> clusterCheckIn(Connection conn)
        ...
        try {
            ...
            // check in...
            lastCheckin = System.currentTimeMillis();
            if (getDelegate().updateSchedulerState(conn, getInstanceId(),
                    lastCheckin) == 0) {
            ...
            throw new JobPersistenceException(
                    "Failure updating scheduler state when checking-in: "
                            + e.getMessage(), e);
        }
    ...
    protected void clusterRecover(Connection conn,
        ...
        if (failedInstances.size() > 0) {
            ...
            try {
                ...
                while (itr.hasNext()) {
                    SchedulerStateRecord rec = itr.next();
                    ...
                    for (FiredTriggerRecord ftRec : firedTriggerRecs) {
                        ...
                        if (ftRec.getFireInstanceState().equals(STATE_BLOCKED)) {
                            getDelegate()
                                    .updateTriggerStatesForJobFromOtherState(
                                            conn, jKey, STATE_WAITING,
                                            STATE_BLOCKED);
                        } else if (ftRec.getFireInstanceState().equals(
                                STATE_PAUSED_BLOCKED)) {
                            getDelegate()
                                    .updateTriggerStatesForJobFromOtherState(
                                            conn, jKey, STATE_PAUSED,
                                            STATE_PAUSED_BLOCKED);
                        }
                        ...
                        if (ftRec.getFireInstanceState().equals(STATE_ACQUIRED)) {
                            getDelegate().updateTriggerStateFromOtherState(
                                    conn, tKey, STATE_WAITING, STATE_ACQUIRED);
                            acquiredCount++;
                        } else if (ftRec.isJobRequestsRecovery()) {
                            // handle jobs marked for recovery that were not
                            // fully
                            // executed..
                            if (jobExists(conn, jKey)) {
                                ...
                                JobDataMap jd = getDelegate()
                                        .selectTriggerJobDataMap(conn,
                                                tKey.getName(), tKey.getGroup());
                                jd.put(Scheduler.FAILED_JOB_ORIGINAL_TRIGGER_NAME,
                                        tKey.getName());
                                jd.put(Scheduler.FAILED_JOB_ORIGINAL_TRIGGER_GROUP,
                                        tKey.getGroup());
                    ...
                    // Check if any of the fired triggers we just deleted were
                    // the last fired trigger
                    ...
                    for (TriggerKey triggerKey : triggerKeys) {
                        ...
                        if (getDelegate().selectTriggerState(conn, triggerKey)
                            ...
                            List<FiredTriggerRecord> firedTriggers = getDelegate()
                                    .selectFiredTriggerRecords(conn,
                                            triggerKey.getName(),
                                            triggerKey.getGroup());
     ...
     * the exception lest it mask the exception that caused the caller to need
     * to rollback in the first place.
     * </p>
     * 
     * @param conn
     *            (Optional)
    ...
    protected void rollbackConnection(Connection conn) {
        if (conn != null) {
            try {
                ...
                getLog().error(
                        "Couldn't rollback jdbc connection. " + e.getMessage(),
                        e);
            }
        }
    }
     ...
     *            (Optional)
     * @throws JobPersistenceException
     *             thrown if a SQLException occurs when the connection is
     *             committed
     */
     ...
     * return null.
     * 
     * @see JobStoreSupport#executeInNonManagedTXLock(String,
     *      TransactionCallback)
     ...
     */
    public Object executeWithoutLock(TransactionCallback txCallback)
            throws JobPersistenceException {
        return executeInLock(null, txCallback);
    }

    /**
     * Execute the given callback having aquired the given lock. Depending on
     * the JobStore, the surrounding transaction may be assumed to be already
     * present (managed). This version is just a handy wrapper around
     * executeInLock that doesn't require a return value.
     * 
     * @param lockName
     *            The name of the lock to aquire, for example "TRIGGER_ACCESS".
     *            If null, then no lock is aquired, but the lockCallback is
     *            still executed in a transaction.
     * 
     * @see #executeInLock(String, TransactionCallback)
     */
    protected void executeInLock(final String lockName,
            final VoidTransactionCallback txCallback)
            throws JobPersistenceException {
    ...
    /**
     * Execute the given callback having optionally aquired the given lock. This
     * uses the non-managed transaction connection. This version is just a handy
     * wrapper around executeInNonManagedTXLock that doesn't require a return
     * value.
     * 
     * @param lockName
     *            The name of the lock to aquire, for example "TRIGGER_ACCESS".
     *            If null, then no lock is aquired, but the lockCallback is
     *            still executed in a non-managed transaction.
     * 
     * @see #executeInNonManagedTXLock(String, TransactionCallback)
     */
    protected void executeInNonManagedTXLock(final String lockName,
            final VoidTransactionCallback txCallback)
            throws JobPersistenceException {
    ...
    class ClusterManager extends Thread {
        ...
        ClusterManager() {
            ...
            this.setName("QuartzScheduler_" + instanceName + "-" + instanceId
                    + "_ClusterManager");
        ...
        public void run() {
            while (!shutdown) {
                ...
                if (!shutdown) {
                    ...
                    if (numFails > 0) {
                        timeToSleep = Math.max(getDbRetryInterval(),
                                timeToSleep);
                    }
    ...
    class MisfireHandler extends Thread {
        ...
        public void run() {
            ...
            while (!shutdown) {
                ...
                if (recoverMisfiredJobsResult
                        .getProcessedMisfiredTriggerCount() > 0) {
                    signalSchedulingChangeImmediately(recoverMisfiredJobsResult
                            .getEarliestNewTime());
                }

                if (!shutdown) {
                    long timeToSleep = 50l; // At least a short pause to help
                                            // balance threads
                    if (!recoverMisfiredJobsResult.hasMoreMisfiredTriggers()) {
                        timeToSleep = getMisfireThreshold()
