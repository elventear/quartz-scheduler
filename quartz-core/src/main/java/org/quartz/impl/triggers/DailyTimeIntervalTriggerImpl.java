 ...
 * minutes or hours (see {@link #setRepeatIntervalUnit(IntervalUnit)}) during a
 ...
 * startTime has no affect. Else if startTime is after startTimeOfDay, then the
 * first fire time for that day will be the next interval after the startTime.
 * For example, if you set startingTimeOfDay=9am, endingTimeOfDay=11am,
 * interval=15 mins, and startTime=9:33am, then the next fire time will be
 * 9:45pm. Note also that if you do not set startTime value, the trigger builder
 * will default to current time, and current time maybe before or after the
 ...
 */
public class DailyTimeIntervalTriggerImpl extends
        AbstractTrigger<DailyTimeIntervalTrigger> implements
    ...
    /**
     * <p>
     * Create a <code>DailyTimeIntervalTrigger</code> that will occur
     * immediately, and repeat at the the given interval.
     * </p>
     * 
     ...
     *            occurring.
     * @param endTimeOfDay
     *            The <code>TimeOfDay</code> that the repeating should stop
     *            occurring.
     * @param intervalUnit
     *            The repeat interval unit. The only intervals that are valid
     *            for this type of trigger are {@link IntervalUnit#SECOND},
     *            {@link IntervalUnit#MINUTE}, and {@link IntervalUnit#HOUR}.
     * @throws IllegalArgumentException
     *             if an invalid IntervalUnit is given, or the repeat interval
     *             is zero or less.
     */
    public DailyTimeIntervalTriggerImpl(String name, TimeOfDay startTimeOfDay,
            TimeOfDay endTimeOfDay, IntervalUnit intervalUnit,
            int repeatInterval) {
        this(name, null, startTimeOfDay, endTimeOfDay, intervalUnit,
                repeatInterval);
    }
     ...
     *             is zero or less.
     */
    public DailyTimeIntervalTriggerImpl(String name, String group,
            TimeOfDay startTimeOfDay, TimeOfDay endTimeOfDay,
            IntervalUnit intervalUnit, int repeatInterval) {
        this(name, group, new Date(), null, startTimeOfDay, endTimeOfDay,
                intervalUnit, repeatInterval);
    }

    /**
     * <p>
     * Create a <code>DailyTimeIntervalTrigger</code> that will occur at the
     * given time, and repeat at the the given interval until the given end
     * time.
     * </p>
     * 
     * @param startTime
    ...
    public DailyTimeIntervalTriggerImpl(String name, Date startTime,
            ...
            IntervalUnit intervalUnit, int repeatInterval) {
        this(name, null, startTime, endTime, startTimeOfDay, endTimeOfDay,
                intervalUnit, repeatInterval);
    }
     ...
     */
    public DailyTimeIntervalTriggerImpl(String name, String group,
            Date startTime, Date endTime, TimeOfDay startTimeOfDay,
            TimeOfDay endTimeOfDay, IntervalUnit intervalUnit,
            int repeatInterval) {
        super(name, group);
     ...
     * <p>
     * Create a <code>DailyTimeIntervalTrigger</code> that will occur at the
     * given time, fire the identified <code>Job</code> and repeat at the the
     ...
     */
    public DailyTimeIntervalTriggerImpl(String name, String group,
            String jobName, String jobGroup, Date startTime, Date endTime,
            TimeOfDay startTimeOfDay, TimeOfDay endTimeOfDay,
            IntervalUnit intervalUnit, int repeatInterval) {
     ...
     * <p>
     * Set the time at which the <code>DailyTimeIntervalTrigger</code> should
     * quit repeating (and be automatically deleted).
     * </p>
     * 
     * @exception IllegalArgumentException
    ...
    public void setRepeatIntervalUnit(IntervalUnit intervalUnit) {
        if (repeatIntervalUnit == null
                || !((repeatIntervalUnit.equals(IntervalUnit.SECOND)
                        || repeatIntervalUnit.equals(IntervalUnit.MINUTE) || repeatIntervalUnit
                            .equals(IntervalUnit.HOUR))))
            throw new IllegalArgumentException(
    ...
    public Date computeFirstFireTime(org.quartz.Calendar calendar) {
        Date sTime = getStartTime();
        Date startTimeOfDayDate = getStartTimeOfDay()
                .getTimeOfDayForDate(sTime);
    ...
    /**
     * <p>
     * Set the next time at which the <code>DailyTimeIntervalTrigger</code>
     * should fire.
     * </p>
     * 
     * <p>
    ...
    public Date getFireTimeAfter(Date afterTime) {
        ...
        // a. Increment afterTime by a second, so that we are comparing against
        // a time after it!
        if (afterTime == null) {
        ...
        // b.Check to see if afterTime is after endTimeOfDay or not. If yes,
        // then we need to advance to next day as well.
        boolean afterTimePassEndTimeOfDay = false;
        if (endTimeOfDay != null) {
            afterTimePassEndTimeOfDay = afterTime.getTime() > endTimeOfDay
                    .getTimeOfDayForDate(afterTime).getTime();
        }
        Date fireTime = advanceToNextDayOfWeek(afterTime,
                afterTimePassEndTimeOfDay);
        if (fireTime == null)
            return null;
        ...
        if (endTimeOfDay == null)
            fireTimeEndDate = new TimeOfDay(23, 59, 59)
                    .getTimeOfDayForDate(fireTime);
        else
        ...
        if (fireTime.getTime() < startTime.getTime()
                && startTime.getTime() < fireTimeStartDate.getTime()) {
            return fireTimeStartDate;
        } else if (fireTime.getTime() < startTime.getTime()
                && startTime.getTime() > fireTimeStartDate.getTime()) {
            return startTime;
        } else if (fireTime.getTime() > startTime.getTime()
                && fireTime.getTime() < fireTimeStartDate.getTime()) {
            return fireTimeStartDate;
        }
        ...
        // f. Continue to calculate the fireTime by incremental unit of
        // intervals.
        ...
        if (repeatUnit.equals(IntervalUnit.SECOND)) {
            ...
            if (secondsAfterStart % (repeatLong * 60L * 60L) != 0)
                jumpCount++;
            sTime.add(Calendar.HOUR_OF_DAY, getRepeatInterval()
                    * (int) jumpCount);
            fireTime = sTime.getTime();
        }

        // g. Ensure this new fireTime is within one day, or else we need to
        // advance to next day.
        if (fireTime.getTime() > fireTimeEndDate.getTime()) {
            // Check to see if fireTime has pass fireTime's end of day. If not,
            // we need to advance by one day.
            Date fireTimeEndOfDay = new TimeOfDay(23, 59, 59)
                    .getTimeOfDayForDate(fireTimeEndDate);
            ...
            // Check to see if next day fireTime is before startTimeOfDay, if
            // not, we need to set to startTimeOfDay.
            Date nextDayfireTimeStartDate = getStartTimeOfDay()
                    .getTimeOfDayForDate(fireTime);
    ...
    /**
     * Given fireTime time, we need to advance/calculate and return a time of
     * next available week day.
     * 
     * @param fireTime
     *            - given next fireTime.
     * @param forceToAdvanceNextDay
     *            - flag to whether to advance day without check existing week
     *            day. This scenario can happen when a caller determine fireTime
     *            has passed the endTimeOfDay that fireTime should move to next
     *            day anyway.
     * @return a next day fireTime.
     */
    private Date advanceToNextDayOfWeek(Date fireTime,
            boolean forceToAdvanceNextDay) {
        // a. Advance or adjust to next dayOfWeek if need to first, starting
        // next day with startTimeOfDay.
        ...
        int dayOfWeekOfFireTime = fireTimeStartDateCal
                .get(Calendar.DAY_OF_WEEK);

        // b2. We need to advance to another day if isAfterTimePassEndTimeOfDay
        // is true, or dayOfWeek is not set.
        Set<Integer> daysOfWeekToFire = getDaysOfWeek();
        if (forceToAdvanceNextDay
                || !daysOfWeekToFire.contains(dayOfWeekOfFireTime)) {
            // Advance one day at a time until next available date.
            for (int i = 1; i <= 7; i++) {
                fireTimeStartDateCal.add(Calendar.DATE, 1);
                dayOfWeekOfFireTime = fireTimeStartDateCal
                        .get(Calendar.DAY_OF_WEEK);
    ...
    public void validate() throws SchedulerException {
        ...
        if (repeatIntervalUnit == IntervalUnit.SECOND
                && repeatInterval > secondsInHour) {
            throw new SchedulerException(
                    "repeatInterval can not exceed 24 hours (" + secondsInHour
                            + " seconds). Given " + repeatInterval);
        }
        if (repeatIntervalUnit == IntervalUnit.MINUTE
                && repeatInterval > secondsInHour / 60L) {
            throw new SchedulerException(
                    "repeatInterval can not exceed 24 hours (" + secondsInHour
                            / 60L + " minutes). Given " + repeatInterval);
        }
    ...
    public void setDaysOfWeek(Set<Integer> daysOfWeek) {
        if (daysOfWeek == null || daysOfWeek.size() == 0)
            ...
            throw new IllegalArgumentException(
                    "DaysOfWeek set must contain at least one day.");
    ...
    public void setStartTimeOfDay(TimeOfDay startTimeOfDay) {
        ...
        if (eTime != null && startTimeOfDay != null
                && eTime.before(startTimeOfDay)) {
            throw new IllegalArgumentException(
    ...
    public void setEndTimeOfDay(TimeOfDay endTimeOfDay) {
        ...
        if (sTime != null && endTimeOfDay != null
                && endTimeOfDay.before(endTimeOfDay)) {
            throw new IllegalArgumentException(
                    "End time of day cannot be before start time of day");
        }
    ...
    public ScheduleBuilder<DailyTimeIntervalTrigger> getScheduleBuilder() {
        ...
        DailyTimeIntervalScheduleBuilder cb = DailyTimeIntervalScheduleBuilder
