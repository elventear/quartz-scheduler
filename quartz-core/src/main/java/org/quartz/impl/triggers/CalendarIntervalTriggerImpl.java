...
/**
 * <p>
 * A concrete <code>{@link Trigger}</code> that is used to fire a <code>{@link org.quartz.JobDetail}</code> based upon
 * repeating calendar time intervals.
 * </p>
 * <p>
 * The trigger will fire every N (see {@link #setRepeatInterval(int)} ) units of calendar time (see
 * {@link #setRepeatIntervalUnit(IntervalUnit)}) as specified in the trigger's definition. This trigger can achieve
 * schedules that are not possible with {@link SimpleTrigger} (e.g because months are not a fixed number of seconds) or
 * {@link CronTrigger} (e.g. because "every 5 months" is not an even divisor of 12).
 * </p>
 * <p>
 * If you use an interval unit of <code>MONTH</code> then care should be taken when setting a <code>startTime</code>
 * value that is on a day near the end of the month. For example, if you choose a start time that occurs on January
 * 31st, and have a trigger with unit <code>MONTH</code> and interval <code>1</code>, then the next fire time will be
 * February 28th, and the next time after that will be March 28th - and essentially each subsequent firing will occur on
 * the 28th of the month, even if a 31st day exists. If you want a trigger that always fires on the last day of the
 * month - regardless of the number of days in the month, you should use <code>CronTrigger</code>.
 * </p>
 * 
 * @see Trigger
 * @see CronTrigger
 ...
 * @author James House
 */
public class CalendarIntervalTriggerImpl extends AbstractTrigger<CalendarIntervalTrigger> implements
    CalendarIntervalTrigger, CoreTrigger {
  ...
  /**
   * <p>
   * Create a <code>DateIntervalTrigger</code> that will occur at the given time, and repeat at the the given interval
   ...
   */
  public CalendarIntervalTriggerImpl(String name, Date startTime, Date endTime, IntervalUnit intervalUnit,
                                     int repeatInterval) {
   ...
   * @param repeatInterval The number of milliseconds to pause between the repeat firing.
   */
  public CalendarIntervalTriggerImpl(String name, String group, Date startTime, Date endTime,
  ...
  /**
   * <p>
   * Create a <code>DateIntervalTrigger</code> that will occur at the given time, fire the identified <code>Job</code>
   ...
   * @param repeatInterval The number of milliseconds to pause between the repeat firing.
   */
  public CalendarIntervalTriggerImpl(String name, String group, String jobName, String jobGroup, Date startTime,
  ...
  /**
   * <p>
   * Set the time at which the <code>DateIntervalTrigger</code> should quit repeating (and be automatically deleted).
   * </p>
   * 
  ...
  /**
   * <p>
   * set the the time interval that will be added to the <code>DateIntervalTrigger</code>'s fire time (in the set repeat
  ...
  /**
   * If intervals are a day or greater, this property (set to true) will cause the firing of the trigger to always occur
   * at the same time of day, (the time of day of the startTime) regardless of daylight saving time transitions. Default
   * value is false.
   * <p>
   * For example, without the property set, your trigger may have a start time of 9:00 am on March 1st, and a repeat
   * interval of 2 days. But after the daylight saving transition occurs, the trigger may start firing at 8:00 am every
   * other day.
   * </p>
   * <p>
   * If however, the time of day does not exist on a given day to fire (e.g. 2:00 am in the United States on the days of
   * daylight saving transition), the trigger will go ahead and fire one hour off on that day, and then resume the
   * normal hour on other days. If you wish for the trigger to never fire at the "wrong" hour, then you should set the
   * property skipDayIfHourDoesNotExist.
   * </p>
   * 
  ...
  /**
   * If intervals are a day or greater, and preserveHourOfDayAcrossDaylightSavings property is set to true, and the hour
   * of the day does not exist on a given day for which the trigger would fire, the day will be skipped and the trigger
   * advanced a second interval if this property is set to true. Defaults to false.
   * <p>
   * <b>CAUTION!</b> If you enable this property, and your hour of day happens to be that of daylight savings transition
   * (e.g. 2:00 am in the United States) and the trigger's interval would have had the trigger fire on that day, then
   * you may actually completely miss a firing on the day of transition if that hour of day does not exist on that day!
   * In such a case the next fire time of the trigger will be computed as double (if the interval is 2 days, then a span
   * of 4 days between firings will occur).
   * </p>
   * 
  ...
  /**
   * <p>
   * Updates the <code>DateIntervalTrigger</code>'s state based on the MISFIRE_INSTRUCTION_XXX that was selected when
   * the <code>DateIntervalTrigger</code> was created.
   * </p>
   * <p>
  ...
  /**
   * <p>
   * Returns the previous time at which the <code>DateIntervalTrigger</code> fired. If the trigger has not yet fired,
  ...
  /**
   * <p>
   * Returns the next time at which the <code>DateIntervalTrigger</code> will fire, after the given time. If the trigger
   * will not fire after the given time, <code>null</code> will be returned.
   * </p>
   */
  @Override
  ...
  protected Date getFireTimeAfter(Date afterTime, boolean ignoreEndTime) {
    ...
    if (!ignoreEndTime && (endMillis <= afterMillis)) { return null; }
    ...
    if (!ignoreEndTime && (endMillis <= time.getTime())) { return null; }
  ...
  private boolean daylightSavingHourShiftOccuredAndAdvanceNeeded(Calendar newTime, int initialHourOfDay) {
    if (isPreserveHourOfDayAcrossDaylightSavings() && newTime.get(Calendar.HOUR_OF_DAY) != initialHourOfDay) {
      ...
      if (newTime.get(Calendar.HOUR_OF_DAY) != initialHourOfDay) return isSkipDayIfHourDoesNotExist();
    }
    return false;
  }
  ...
  /**
   * <p>
   * Returns the final time at which the <code>DateIntervalTrigger</code> will fire, if there is no end time set, null
   * will be returned.
   * </p>
   * <p>
