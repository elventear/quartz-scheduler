 ...
 * calendar time (see {@link #setRepeatIntervalUnit(IntervalUnit)}) as specified
 * in the trigger's definition. This trigger can achieve schedules that are not
 ...
 * @author James House
 */
public class CalendarIntervalTriggerImpl extends
        AbstractTrigger<CalendarIntervalTrigger> implements
    ...
    /**
     * <p>
     * Create a <code>DateIntervalTrigger</code> that will occur immediately,
     * and repeat at the the given interval.
     * </p>
     */
    public CalendarIntervalTriggerImpl(String name, IntervalUnit intervalUnit,
            int repeatInterval) {
     ...
     * </p>
     */
    public CalendarIntervalTriggerImpl(String name, String group,
     ...
     */
    public CalendarIntervalTriggerImpl(String name, String group,
            Date startTime, Date endTime, IntervalUnit intervalUnit,
            int repeatInterval) {
        super(name, group);
     ...
     * daylight saving transition occurs, the trigger may start firing at 8:00
     * am every other day.
     * </p>
     * 
     * <p>
     ...
     * resume the normal hour on other days. If you wish for the trigger to
     * never fire at the "wrong" hour, then you should set the property
     ...
     * fire, the day will be skipped and the trigger advanced a second interval
     * if this property is set to true. Defaults to false.
     * 
     * <p>
    ...
    /**
     * <p>
     * Set the previous time at which the <code>DateIntervalTrigger</code>
     * fired.
     * </p>
     * 
     * <p>
    ...
    protected Date getFireTimeAfter(Date afterTime, boolean ignoreEndTime) {
        ...
        if (getRepeatIntervalUnit().equals(IntervalUnit.SECOND)) {
            ...
            if (getRepeatIntervalUnit().equals(IntervalUnit.DAY)) {
                ...
                // Because intervals greater than an hour have an non-fixed
                // number
                // of seconds in them (due to daylight savings, variation number
                // of
                // days in each month, leap year, etc. ) we can't jump forward
                // an
                ...
                long jumpCount = secondsAfterStart
                        / (repeatLong * 24L * 60L * 60L);
                // if we need to make a big jump, jump most of the way there,
                // but not all the way because in some cases we may over-shoot
                // or under-shoot
                if (jumpCount > 20) {
                    if (jumpCount < 50)
                    ...
                    sTime.add(java.util.Calendar.DAY_OF_YEAR,
                            (int) (getRepeatInterval() * jumpCount));
                }
    ...
    public Date getFinalFireTime() {
        ...
        // otherwise we have to back up one interval from the fire time after
        // the end time
    ...
    public ScheduleBuilder<CalendarIntervalTrigger> getScheduleBuilder() {
        ...
        CalendarIntervalScheduleBuilder cb = CalendarIntervalScheduleBuilder
                .calendarIntervalSchedule().withInterval(getRepeatInterval(),
