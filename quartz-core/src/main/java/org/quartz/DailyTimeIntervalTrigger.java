...
public interface DailyTimeIntervalTrigger extends Trigger {
     ...
     * situation, the <code>{@link DailyIntervaledTimeRangeTrigger}</code> wants
     ...
     * Instructs the <code>{@link Scheduler}</code> that upon a mis-fire
     * situation, the <code>{@link DailyIntervaledTimeRangeTrigger}</code> wants
     * to have it's next-fire-time updated to the next time in the schedule
     * after the current time (taking into account any associated
     ...
     *         per the values 1-7 as defined by
