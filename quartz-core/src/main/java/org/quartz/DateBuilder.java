...
/**
 * <code>DateBuilder</code> is used to conveniently create <code>java.util.Date</code> instances that meet particular
 * criteria.
 * <p>
 * Quartz provides a builder-style API for constructing scheduling-related entities via a Domain-Specific Language
 * (DSL). The DSL can best be utilized through the usage of static imports of the methods on the classes
 * <code>TriggerBuilder</code>, <code>JobBuilder</code>, <code>DateBuilder</code>, <code>JobKey</code>,
 * <code>TriggerKey</code> and the various <code>ScheduleBuilder</code> implementations.
 * </p>
 * <p>
...
public class DateBuilder {
  ...
  public Date build() {
    ...
    if (tz != null && lc != null) cal = Calendar.getInstance(tz, lc);
    else if (tz != null) cal = Calendar.getInstance(tz);
    else if (lc != null) cal = Calendar.getInstance(lc);
   ...
   * </p>
   * <p>
   * For example a current time of 08:13:54 would result in a date with the time of 09:00:00. If the date's time is in
   * the 23rd hour, the date's 'day' will be promoted, and the time will be set to 00:00:00.
   * </p>
   * 
   ...
   * </p>
   * <p>
   * For example an input date with a time of 08:13:54 would result in a date with the time of 09:00:00. If the date's
   * time is in the 23rd hour, the date's 'day' will be promoted, and the time will be set to 00:00:00.
   * </p>
   * 
   ...
   * </p>
   * <p>
   * For example a current time of 08:13:54 would result in a date with the time of 08:14:00. If the date's time is in
   * the 59th minute, then the hour (and possibly the day) will be promoted.
   * </p>
   * 
   * @return the new rounded date
   */
   ...
   * </p>
   * <p>
   * For example an input date with a time of 08:13:54 would result in a date with the time of 08:14:00. If the date's
   * time is in the 59th minute, then the hour (and possibly the day) will be promoted.
   * </p>
   * 
   * @param date the Date to round, if <code>null</code> the current time will be used
   * @return the new rounded date
   ...
   * </p>
   * <p>
   * For example an input date with a time of 08:13:54, and an input minute-base of 5 would result in a date with the
   * time of 08:15:00. The same input date with an input minute-base of 10 would result in a date with the time of
   * 08:20:00. But a date with the time 08:53:31 and an input minute-base of 45 would result in 09:00:00, because the
   ...
   * <td>11:16:41</td>
   * <td>17</td>
   * <td>11:17:00</td> </tr> </tr>
   * <td>11:17:41</td>
   * <td>17</td>
   * <td>11:34:00</td> </tr> </tr>
   * <td>11:52:41</td>
   * <td>17</td>
   * <td>12:00:00</td> </tr> </tr>
   * <td>11:52:41</td>
   * <td>5</td>
   * <td>11:55:00</td> </tr> </tr>
   * <td>11:57:41</td>
   * <td>5</td>
   * <td>12:00:00</td> </tr> </tr>
   * <td>11:17:41</td>
  ...
  public static Date nextGivenMinuteDate(Date date, int minuteBase) {
    if (minuteBase < 0 || minuteBase > 59) { throw new IllegalArgumentException("minuteBase must be >=0 and <= 59"); }
  ...
  public static Date nextGivenSecondDate(Date date, int secondBase) {
    if (secondBase < 0 || secondBase > 59) { throw new IllegalArgumentException("secondBase must be >=0 and <= 59"); }
  ...
  /**
   * Translate a date & time from a users time zone to the another (probably server) time zone to assist in creating a
   * simple trigger with the right date & time.
   * 
  ...
  public static void validateDayOfWeek(int dayOfWeek) {
    if (dayOfWeek < SUNDAY || dayOfWeek > SATURDAY) { throw new IllegalArgumentException("Invalid day of week."); }
  }

  public static void validateHour(int hour) {
    if (hour < 0 || hour > 23) { throw new IllegalArgumentException("Invalid hour (must be >= 0 and <= 23)."); }
  }

  public static void validateMinute(int minute) {
    if (minute < 0 || minute > 59) { throw new IllegalArgumentException("Invalid minute (must be >= 0 and <= 59)."); }
  }

  public static void validateSecond(int second) {
    if (second < 0 || second > 59) { throw new IllegalArgumentException("Invalid second (must be >= 0 and <= 59)."); }
  }

  public static void validateDayOfMonth(int day) {
    if (day < 1 || day > 31) { throw new IllegalArgumentException("Invalid day of month."); }
  }

  public static void validateMonth(int month) {
    if (month < 1 || month > 12) { throw new IllegalArgumentException("Invalid month (must be >= 1 and <= 12."); }
  }

  private static final int MAX_YEAR = Calendar.getInstance().get(Calendar.YEAR) + 100;

  public static void validateYear(int year) {
    if (year < 0 || year > MAX_YEAR) { throw new IllegalArgumentException("Invalid year (must be >= 0 and <= "
                                                                          + MAX_YEAR); }
  }
