...
/*
 * Copyright 2001-2009 Terracotta, Inc. Licensed under the Apache License, Version 2.0 (the "License"); you may not use
...
/**
 * <p>
 * This is the heart of Quartz, an indirect implementation of the <code>{@link org.quartz.Scheduler}</code> interface,
 * containing methods to schedule <code>{@link org.quartz.Job}</code>s, register
...
public class QuartzScheduler implements RemotableQuartzScheduler {
  ...
  private final long                             dbRetryInterval;
  ...
  public QuartzScheduler(QuartzSchedulerResources resources, long idleWaitTime, long dbRetryInterval)
    ...
    if (shouldRunUpdateCheck()) updateTimer = scheduleUpdateCheck();
  ...
  public void initialize() throws SchedulerException {
    ...
    getLog().info("Scheduler meta-data: "
                      + (new SchedulerMetaData(getSchedulerName(), getSchedulerInstanceId(), getClass(), boundRemotely,
                                               runningSince() != null, isInStandbyMode(), isShutdown(), runningSince(),
                                               numJobsExecuted(), getJobStoreClass(), supportsPersistence(),
                                               isClustered(), getThreadPoolClass(), getThreadPoolSize(), getVersion()))
                          .toString());
  }
  ...
  private boolean shouldRunUpdateCheck() {
    if (resources.isRunUpdateCheck() && !Boolean.getBoolean(StdSchedulerFactory.PROP_SCHED_SKIP_UPDATE_CHECK)
        && !Boolean.getBoolean("org.terracotta.quartz.skipUpdateCheck")) { return true; }
    return false;
  }
   ...
   * </p>
   * <p>
   * All <code>{@link org.quartz.Trigger}s</code> that have misfired will be passed to the appropriate
   * TriggerListener(s).
   * </p>
   */
  ...
  /**
   * <p>
   * Halts the <code>QuartzScheduler</code>'s firing of <code>{@link org.quartz.Trigger}s</code>, and cleans up all
   * resources associated with the QuartzScheduler. Equivalent to <code>shutdown(false)</code>.
   * </p>
   * <p>
   ...
   * </p>
   * 
   * @param waitForJobsToComplete if <code>true</code> the scheduler will not allow this method to return until all
  ...
  public void shutdown(boolean waitForJobsToComplete) {
    ...
    if ((resources.isInterruptJobsOnShutdown() && !waitForJobsToComplete)
      ...
      for (JobExecutionContext job : jobs) {
        if (job.getJobInstance() instanceof InterruptableJob) try {
          ((InterruptableJob) job.getJobInstance()).interrupt();
        } catch (Throwable e) {
          // do nothing, this was just a courtesy effort
          getLog().warn("Encountered error when interrupting job {} during shutdown: {}", job.getJobDetail().getKey(),
                        e);
        }
      }
    }
  ...
  public void validateState() throws SchedulerException {
    if (isShutdown()) { throw new SchedulerException("The Scheduler has been shutdown."); }
  ...
  /**
   * <p>
   * Return a list of <code>JobExecutionContext</code> objects that represent all currently executing Jobs in this
   * Scheduler instance.
   * </p>
   * <p>
   * This method is not cluster aware. That is, it will only return Jobs currently executing in this Scheduler instance,
   * not across the entire cluster.
   * </p>
   * <p>
   * Note that the list returned is an 'instantaneous' snap-shot, and that as soon as it's returned, the true list of
   * executing jobs may be different.
   * </p>
   */
  ...
  /**
   * <p>
   * Add the <code>{@link org.quartz.Job}</code> identified by the given <code>{@link org.quartz.JobDetail}</code> to
   * the Scheduler, and associate the given <code>{@link org.quartz.Trigger}</code> with it.
   * </p>
   * <p>
   * If the given Trigger does not reference any <code>Job</code>, then it will be set to reference the Job passed with
   * it into this method.
   * </p>
   * 
   * @throws SchedulerException if the Job or Trigger cannot be added to the Scheduler, or there is an internal
   *         Scheduler error.
   */
  public Date scheduleJob(JobDetail jobDetail, Trigger trigger) throws SchedulerException {
    validateState();

    if (jobDetail == null) { throw new SchedulerException("JobDetail cannot be null"); }

    if (trigger == null) { throw new SchedulerException("Trigger cannot be null"); }

    if (jobDetail.getKey() == null) { throw new SchedulerException("Job's key cannot be null"); }
    ...
    } else if (!trigger.getJobKey().equals(jobDetail.getKey())) { throw new SchedulerException(
    ...
    if (ft == null) { throw new SchedulerException("Based on configured schedule, the given trigger '"
  ...
  /**
   * <p>
   * Schedule the given <code>{@link org.quartz.Trigger}</code> with the <code>Job</code> identified by the
   * <code>Trigger</code>'s settings.
   * </p>
   * 
   * @throws SchedulerException if the indicated Job does not exist, or the Trigger cannot be added to the Scheduler, or
  ...
  public Date scheduleJob(Trigger trigger) throws SchedulerException {
    ...
    if (trigger.getCalendarName() != null) {
      cal = resources.getJobStore().retrieveCalendar(trigger.getCalendarName());
      if (cal == null) { throw new SchedulerException("Calendar not found: " + trigger.getCalendarName()); }
    }
    Date ft = trig.computeFirstFireTime(cal);
  ...
  /**
   * <p>
   * Add the given <code>Job</code> to the Scheduler - with no associated <code>Trigger</code>. The <code>Job</code>
   * will be 'dormant' until it is scheduled with a <code>Trigger</code>, or <code>Scheduler.triggerJob()</code> is
   * called for it.
   * </p>
   * <p>
   ...
   * </p>
   * 
   * @throws SchedulerException if there is an internal Scheduler error, or if the Job is not durable, or a Job with the
   *         same name already exists, and <code>replace</code> is <code>false</code>.
   */
  public void addJob(JobDetail jobDetail, boolean replace) throws SchedulerException {
    validateState();

    if (!jobDetail.isDurable() && !replace) { throw new SchedulerException(
  ...
  public boolean deleteJob(JobKey jobKey) throws SchedulerException {
    ...
    for (Trigger trigger : triggers) {
      if (!unscheduleJob(trigger.getKey())) {
        StringBuilder sb = new StringBuilder().append("Unable to unschedule trigger [").append(trigger.getKey())
            .append("] while deleting job [").append(jobKey).append("]");
  ...
  public void scheduleJobs(Map<JobDetail, List<Trigger>> triggersAndJobs, boolean replace) throws SchedulerException {
    ...
    for (Entry<JobDetail, List<Trigger>> e : triggersAndJobs.entrySet()) {
      ...
      for (Trigger trigger : triggers) {
        ...
        if (trigger.getCalendarName() != null) {
          cal = resources.getJobStore().retrieveCalendar(trigger.getCalendarName());
          if (cal == null) { throw new SchedulerException("Calendar '" + trigger.getCalendarName()
                                                          + "' not found for trigger: " + trigger.getKey()); }
        }
        Date ft = opt.computeFirstFireTime(cal);

        if (ft == null) { throw new SchedulerException(
  ...
  /**
   * <p>
   * Remove (delete) the <code>{@link org.quartz.Trigger}</code> with the given name, and store the new given one -
   * which must be associated with the same job.
   * </p>
   * 
   * @param newTrigger The new <code>Trigger</code> to be stored.
   * @return <code>null</code> if a <code>Trigger</code> with the given name & group was not found and removed from the
   *         store, otherwise the first fire time of the newly scheduled trigger.
   */
  public Date rescheduleJob(TriggerKey triggerKey, Trigger newTrigger) throws SchedulerException {
    validateState();

    if (triggerKey == null) { throw new IllegalArgumentException("triggerKey cannot be null"); }
    if (newTrigger == null) { throw new IllegalArgumentException("newTrigger cannot be null"); }
    ...
    if (ft == null) { throw new SchedulerException("Based on configured schedule, the given trigger will never fire."); }
  ...
  public void triggerJob(JobKey jobKey, JobDataMap data) throws SchedulerException {
    ...
    // TODO: use builder
    OperableTrigger trig = new org.quartz.impl.triggers.SimpleTriggerImpl(newTriggerId(), Scheduler.DEFAULT_GROUP,
  ...
  /**
   * <p>
   * Pause all of the <code>{@link org.quartz.JobDetail}s</code> in the matching groups - by pausing all of their
   * <code>Trigger</code>s.
   * </p>
   */
  ...
  /**
   * <p>
   * Pause all triggers - equivalent of calling <code>pauseTriggers(GroupMatcher<TriggerKey>)</code> with a matcher
   ...
   * </p>
   * 
   * @throws SchedulerException if there is an internal Scheduler error, or a Calendar with the same name already
  ...
  public void addInternalJobListener(JobListener jobListener) {
    if (jobListener.getName() == null || jobListener.getName().length() == 0) { throw new IllegalArgumentException(
  ...
  /**
   * <p>
   * Remove the identified <code>{@link JobListener}</code> from the <code>Scheduler</code>'s list of <i>internal</i>
   * listeners.
   * </p>
   * 
  ...
  /**
   * <p>
   * Get a List containing all of the <code>{@link org.quartz.JobListener}</code>s in the <code>Scheduler</code>'s
   * <i>internal</i> list.
   * </p>
   */
  ...
  public void addInternalTriggerListener(TriggerListener triggerListener) {
    if (triggerListener.getName() == null || triggerListener.getName().length() == 0) { throw new IllegalArgumentException(
  ...
  /**
   * <p>
   * Remove the identified <code>{@link TriggerListener}</code> from the <code>Scheduler</code>'s list of
   * <i>internal</i> listeners.
   * </p>
   * 
   * @return true if the identified listener was found in the list, and removed.
   */
  ...
  /**
   * <p>
   * Get a list containing all of the <code>{@link org.quartz.TriggerListener}</code>s in the <code>Scheduler</code>'s
   * <i>internal</i> list.
   * </p>
   */
  ...
  /**
   * <p>
   * Register the given <code>{@link SchedulerListener}</code> with the <code>Scheduler</code>'s list of internal
   * listeners.
   * </p>
   */
  ...
  /**
   * <p>
   * Remove the given <code>{@link SchedulerListener}</code> from the <code>Scheduler</code>'s list of internal
   * listeners.
   * </p>
   * 
   * @return true if the identified listener was found in the list, and removed.
  ...
  /**
   * <p>
   * Get a List containing all of the <i>internal</i> <code>{@link SchedulerListener}</code>s registered with the
   * <code>Scheduler</code>.
   * </p>
   */
  ...
  protected void notifyJobStoreJobComplete(OperableTrigger trigger, JobDetail detail,
                                           CompletedExecutionInstruction instCode) throws JobPersistenceException {

    resources.getJobStore().triggeredJobComplete(trigger, detail, instCode);
  }

  protected void notifyJobStoreJobVetoed(OperableTrigger trigger, JobDetail detail,
                                         CompletedExecutionInstruction instCode) throws JobPersistenceException {
  ...
  public boolean notifyTriggerListenersFired(JobExecutionContext jec) throws SchedulerException {
    ...
    for (TriggerListener tl : triggerListeners) {
      try {
        ...
        SchedulerException se = new SchedulerException("TriggerListener '" + tl.getName() + "' threw exception: "
                                                       + e.getMessage(), e);
        throw se;
      }
    }
  ...
  public void notifyTriggerListenersComplete(JobExecutionContext jec, CompletedExecutionInstruction instCode)
      throws SchedulerException {
    // build a list of all trigger listeners that are to be notified...
    List<TriggerListener> triggerListeners = buildTriggerListenerList();
  ...
  public void notifyJobListenersToBeExecuted(JobExecutionContext jec) throws SchedulerException {
    ...
    for (JobListener jl : jobListeners) {
      try {
        ...
        SchedulerException se = new SchedulerException("JobListener '" + jl.getName() + "' threw exception: "
                                                       + e.getMessage(), e);
        throw se;
      }
    }
  ...
  public void notifySchedulerListenersUnscheduled(TriggerKey triggerKey) {
    ...
    for (SchedulerListener sl : schedListeners) {
      try {
        ...
        getLog().error("Error while notifying SchedulerListener of unscheduled job." + "  Triger="
                           + (triggerKey == null ? "ALL DATA" : triggerKey), e);
      }
    }
  }
  ...
  public void notifySchedulerListenersFinalized(Trigger trigger) {
    ...
    for (SchedulerListener sl : schedListeners) {
      try {
        ...
        getLog()
            .error("Error while notifying SchedulerListener of finalized trigger." + "  Triger=" + trigger.getKey(), e);
      }
    }
  }
  ...
  public void setJobFactory(JobFactory factory) throws SchedulerException {
    ...
    if (factory == null) { throw new IllegalArgumentException("JobFactory cannot be set to null!"); }
   ...
   * <p>
   * This method is not cluster aware. That is, it will only interrupt instances of the identified InterruptableJob
   * currently executing in this Scheduler instance, not across the entire cluster.
   * </p>
   * 
    ...
    for (JobExecutionContext jec : jobs) {
      ...
      if (jobKey.equals(jobDetail.getKey())) {
        ...
        if (job instanceof InterruptableJob) {
          ...
          throw new UnableToInterruptJobException("Job " + jobDetail.getKey()
    ...
    for (JobExecutionContext jec : jobs) {
      if (jec.getFireInstanceId().equals(fireInstanceId)) {
        ...
        if (job instanceof InterruptableJob) {
          ...
          throw new UnableToInterruptJobException("Job " + jec.getJobDetail().getKey()
                                                  + " can not be interrupted, since it does not implement "
                                                  + InterruptableJob.class.getName());
        }
      }
