 ...
 * <p>
 * This builder provide an extra convenient method for you to set the trigger's endTimeOfDay. You may use either
 * endingDailyAt() or endingDailyAfterCount() to set the value. The later will auto calculate your endTimeOfDay by using
 * the interval, intervalUnit and startTimeOfDay to perform the calculation.
 * <p>
 * When using endingDailyAfterCount(), you should note that it is used to calculating endTimeOfDay. So if your startTime
 * on the first day is already pass by a time that would not add up to the count you expected, until the next day comes.
 * Remember that DailyTimeIntervalTrigger will use startTimeOfDay and endTimeOfDay as fresh per each day!
 * <p>
 ...
 *                                                                                                                 &quot;myTriggerGroup&quot;))
 *                                                                                        .withSchedule(onDaysOfTheWeek(MONDAY,
 *                                                                                                                      THURSDAY))
...
public class DailyTimeIntervalScheduleBuilder extends ScheduleBuilder<DailyTimeIntervalTrigger> {
  ...
  /**
   * A set of all days of the week. The set contains all values between {@link java.util.Calendar#SUNDAY} and
   * {@link java.util.Calendar#SATURDAY} (the integers from 1 through 7).
   */
  public static final Set<Integer> ALL_DAYS_OF_THE_WEEK;

  /**
   * A set of the business days of the week (for locales similar to the USA). The set contains all values between
   * {@link java.util.Calendar#MONDAY} and {@link java.util.Calendar#FRIDAY} (the integers from 2 through 6).
   */
   ...
   * {@link java.util.Calendar#SATURDAY} and {@link java.util.Calendar#SUNDAY}
   */
  ...
  /**
   * Build the actual Trigger -- NOT intended to be invoked by end users, but will rather be invoked by a TriggerBuilder
   * which this ScheduleBuilder is given to.
   * 
  ...
  public DailyTimeIntervalScheduleBuilder withInterval(int timeInterval, IntervalUnit unit) {
    if (unit == null
        || !(unit.equals(IntervalUnit.SECOND) || unit.equals(IntervalUnit.MINUTE) || unit.equals(IntervalUnit.HOUR))) throw new IllegalArgumentException(
   ...
   * 
   * @param onDaysOfWeek a Set containing the integers representing the days of the week, per the values 1-7 as defined
   *        by {@link java.util.Calendar#SUNDAY} - {@link java.util.Calendar#SATURDAY}.
   * @return the updated DailyTimeIntervalScheduleBuilder
   */
  public DailyTimeIntervalScheduleBuilder onDaysOfTheWeek(Set<Integer> onDaysOfWeek) {
    if (onDaysOfWeek == null || onDaysOfWeek.size() == 0) throw new IllegalArgumentException(
                                                                                             "Days of week must be an non-empty set.");
    for (Integer day : onDaysOfWeek)
      if (!ALL_DAYS_OF_THE_WEEK.contains(day)) throw new IllegalArgumentException("Invalid value for day of week: "
                                                                                  + day);
  ...
  /**
   * Calculate and set the endTimeOfDay using count, interval and starTimeOfDay. This means that these must be set
   * before this method is call.
   * 
   * @return the updated DailyTimeIntervalScheduleBuilder
   */
  public DailyTimeIntervalScheduleBuilder endingDailyAfterCount(int count) {
    ...
    if (startTimeOfDay == null) throw new IllegalArgumentException(
    ...
    if (remainingMillisInDay - intervalInMillis <= 0) throw new IllegalArgumentException(
    ...
    if (count > maxNumOfCount) throw new IllegalArgumentException("The given count " + count
    ...
    if (endTimeOfDayDate.getTime() > maxEndTimeOfDayDate.getTime()) throw new IllegalArgumentException(
                                                                                                       "The given count "
                                                                                                           + count
                                                                                                           + " is too large! The max you can set is "
                                                                                                           + maxNumOfCount);
   ...
   * @return the new DailyTimeIntervalScheduleBuilder
   */
  public DailyTimeIntervalScheduleBuilder withRepeatCount(int count) {
