 ...
 * <code>{@link Trigger}</code> may (not) fire. Calendars do not define actual fire times, but rather are used to limit
 * a <code>Trigger</code> from firing on its normal schedule if necessary. Most Calendars include all times by default
 * and allow the user to specify times to exclude.
 * <p>
...
public interface Calendar extends java.io.Serializable, java.lang.Cloneable {
  ...
  /**
   * <p>
   * Set a description for the <code>Calendar</code> instance - may be useful for remembering/displaying the purpose of
   * the calendar, though the description has no meaning to Quartz.
   * </p>
   */
