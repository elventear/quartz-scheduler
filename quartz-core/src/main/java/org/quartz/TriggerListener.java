...
/**
 * The interface to be implemented by classes that want to be informed when a <code>{@link Trigger}</code> fires. In
 * general, applications that use a <code>Scheduler</code> will not have use for this mechanism.
 * 
...
public interface TriggerListener {
   ...
   * @param context The <code>JobExecutionContext</code> that will be passed to the <code>Job</code>'s
   ...
   * <p>
   * Called by the <code>{@link Scheduler}</code> when a <code>{@link Trigger}</code> has fired, and it's associated
   * <code>{@link org.quartz.JobDetail}</code> is about to be executed. If the implementation vetos the execution (via
   ...
   * </p>
   * <p>
   * Consideration should be given to how much time is spent in this method, as it will affect all triggers that are
   * misfiring. If you have lots of triggers misfiring at once, it could be an issue it this method does a lot.
   * </p>
   * 
   ...
   * <code>{@link org.quartz.JobDetail}</code> has been executed, and it's <code>triggered(xx)</code> method has been
   * called.
   * </p>
   * 
