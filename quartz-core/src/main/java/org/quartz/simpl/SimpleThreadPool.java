 ...
 * </p>
 * <p>
 * <CODE>Runnable</CODE> objects are sent to the pool with the <code>{@link #runInThread(Runnable)}</code> method, which
 * blocks until a <code>Thread</code> becomes available.
 * </p>
 * <p>
...
public class SimpleThreadPool implements ThreadPool {
  ...
  /**
   * <p>
   * Create a new <code>SimpleThreadPool</code> with the specified number of <code>Thread</code> s that have the given
   * priority.
   * </p>
   * 
  ...
  /**
   * <p>
   * Set the thread priority of worker threads in the pool - has no effect after <code>initialize()</code> has been
   * called.
   * </p>
   */
  ...
  public void initialize() throws SchedulerConfigException {
    ...
    if (prio <= 0 || prio > 9) { throw new SchedulerConfigException("Thread priority must be > 0 and <= 9"); }
  ...
  /**
   * <p>
   * Run the given <code>Runnable</code> object in the next available <code>Thread</code>. If while waiting the thread
   * pool is asked to shut down, the Runnable is executed immediately within a new additional thread.
   * </p>
   * 
  ...
  public boolean runInThread(Runnable runnable) {
    ...
    synchronized (nextRunnableLock) {
      ...
      if (!isShutdown) {
        ...
        WorkerThread wt = new WorkerThread(this, threadGroup, "WorkerThread-LastJob", prio, isMakeThreadsDaemons(),
                                           runnable);
        busyWorkers.add(wt);
        workers.add(wt);
        wt.start();
  ...
  class WorkerThread extends Thread {
    ...
    /**
     * <p>
     * Create a worker thread and start it. Waiting for the next Runnable, executing it, and waiting for the next
     * Runnable, until the shutdown flag is set.
     * </p>
     */
    ...
    public void run(Runnable newRunnable) {
      synchronized (this) {
        if (runnable != null) { throw new IllegalStateException("Already running a Runnable!"); }
