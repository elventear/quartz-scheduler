...
/**
 * <p>
 * A <code>ConnectionProvider</code> implementation that creates its own pool of
 * connections.
 * </p>
 * 
 * <p>
 * This class uses C3PO (http://www.mchange.com/projects/c3p0/index.html) as the
...
public class PoolingConnectionProvider implements ConnectionProvider {
    ...
    /**
     * The maximum number of database connections to have in the pool. Default
     * is 10.
     */
    public static final String DB_MAX_CONNECTIONS = "maxConnections";

    /**
     * The maximum number of prepared statements that will be cached per
     * connection in the pool. Depending upon your JDBC Driver this may
     * significantly help performance, or may slightly hinder performance.
     * Default is 120, as Quartz uses over 100 unique statements. 0 disables the
     * feature.
     */
    ...
    /**
     * Discard connections after they have been idle this many seconds. 0
    ...
    public PoolingConnectionProvider(Properties config)
        ...
        initialize(cfg.getStringProperty(DB_DRIVER),
                cfg.getStringProperty(DB_URL), cfg.getStringProperty(DB_USER,
                        ""), cfg.getStringProperty(DB_PASSWORD, ""),
                cfg.getIntProperty(DB_MAX_CONNECTIONS,
                        DEFAULT_DB_MAX_CONNECTIONS), cfg.getIntProperty(
    ...
    /**
     * Create the underlying C3PO ComboPooledDataSource with the default
    ...
    private void initialize(String dbDriver, String dbURL, String dbUser,
            ...
            int maxStatementsPerConnection, String dbValidationQuery,
        ...
        if (dbDriver == null) {
            throw new SQLException("DBPool '" + dbURL
        ...
        try {
            ...
            throw new SchedulerException(
                    "Problem setting driver class name on datasource: "
                            + e.getMessage(), e);
        }
     ...
     * 
     * <p>
     * This can be used to set additional data source properties in a subclass's
     * constructor.
     * </p>
     */
