...
/**
 * <p>
 * An implementation of <code>Map</code> that wraps another <code>Map</code> and flags itself 'dirty' when it is
 * modified, enforces that all keys are Strings.
 * </p>
 * <p>
...
public class StringKeyDirtyFlagMap extends DirtyFlagMap<String, Object> {
  ...
  /**
   * @deprecated JDBCJobStores no longer prune out transient data. If you include non-Serializable values in the Map,
   *             you will now get an exception when attempting to store it in a database.
   */
  @Deprecated
  ...
  /**
   * Tell the <code>StringKeyDirtyFlagMap</code> that it should allow non-<code>Serializable</code> values. Enforces
   * that the Map doesn't already include transient data.
   * 
   * @deprecated JDBCJobStores no longer prune out transient data. If you include non-Serializable values in the Map,
   *             you will now get an exception when attempting to store it in a database.
   */
  ...
  public void setAllowsTransientData(boolean allowsTransientData) {
    ...
    if (containsTransientData() && !allowsTransientData) { throw new IllegalStateException(
  ...
  /**
   * Determine whether any values in this Map do not implement <code>Serializable</code>. Always returns false if this
   * Map is flagged to not allow transient data.
   * 
   * @deprecated JDBCJobStores no longer prune out transient data. If you include non-Serializable values in the Map,
   *             you will now get an exception when attempting to store it in a database.
   */
  ...
  /**
   * Removes any data values in the map that are non-Serializable. Does nothing if this Map does not allow transient
   * data.
   * 
   * @deprecated JDBCJobStores no longer prune out transient data. If you include non-Serializable values in the Map,
   *             you will now get an exception when attempting to store it in a database.
